...
def analysis():
    ...
    btn_daily_order_item_wise = request.vars.btn_daily_order_item_wise
    ...
    btn_sales_research_level0=request.vars.btn_sales_research_level0
    btn_sales_research_level1=request.vars.btn_sales_research_level1
    btn_sales_research_rep=request.vars.btn_sales_research_rep


    # ===========

    btn_invoice_head_d=request.vars.btn_invoice_head_d
    btn_invoice_detail_d=request.vars.btn_invoice_detail_d

    btn_order_head_d=request.vars.btn_order_head_d
    btn_order_detail_d=request.vars.btn_order_detail_d


    if (btn_order_head_d or btn_order_detail_d or btn_invoice_head_d or btn_invoice_detail_d or btn_new_client_summary_d or btn_new_client_summary or btn_new_client_d or btn_new_client or btn_productivity or btn_coverage or btn_daily_sales_item_wise or btn_daily_order_item_wise or btn_customer_sales or btn_customer_sales_d or btn_customer_order or btn_customer_order_d or btn_zero_sales_outlet or btn_missed_outlet or btn_inactive_user or btn_sr_attendance or btn_sr_wise_sales or btn_sr_wise_sales_d or btn_sr_wise_visit or btn_sr_wise_visit_d or btn_dashboard or btn_market_research_level0_pres or btn_market_research_level1_pres or btn_market_research_rep_pres or btn_pres_survey_visit or btn_visit_list or btn_visit_count or btn_market_research_level0 or btn_market_research_level1 or btn_market_research_rep or btn_doctor_visit_summary or btn_schedule_and_completed_visit or btn_client_updated or visit_list_summary or btn_order_details_itemwise or btn_order_details_repwise or btn_order_details_depotwise or btn_sales_summary_itemwise or btn_sales_summary_repwise or btn_sales_summary_depotwise or btn_sales_details_download or btn_doctor_visit or btn_pres_survey_summary or btn_pres_survey_visit or btn_doctor_visit_individual or btn_sales_research_level0 or btn_sales_research_level1 or btn_sales_research_rep):
        ...
        if dateFlag==False:
            ...
            if dateDiff>90:
                ...
                elif btn_daily_order_item_wise:
                    if dateDiff > 1:
                        response.flash = "Single day allowed between Date Range"
                    else:
                        redirect(URL('daily_order_item_wise',vars=dict(fromDate=from_dt, toDate=to_dt, regionValue=regionValue, areaValue=areaValue,territoryValue=territoryValue, marketValue=marketValue, repCM=repCM)))
                ...
                elif btn_sales_research_level0:
                    redirect (URL('sales_research_level0',vars=dict(fromDate=from_dt,toDate=to_dt,regionValue=regionValue,areaValue=areaValue))) 
                
                elif btn_sales_research_level1:
                    redirect (URL('sales_research_level1',vars=dict(fromDate=from_dt,toDate=to_dt,regionValue=regionValue,areaValue=areaValue))) 
                    
                elif btn_sales_research_rep:
                    redirect (URL('sales_research_rep',vars=dict(fromDate=from_dt,toDate=to_dt,regionValue=regionValue,areaValue=areaValue))) 
                ...
                try:
                    ...
                    # to_dt = to_dt2 + datetime.timedelta(days=1)
                    # return to_dt
                ...
                if dateFlag==False:
                    ...
                    if dateDiff>30:                        
                        response.flash="Maximum 30 days allowed between Date Range"
                    else:                        
                        if btn_invoice_head_d:
                            redirect (URL('invoice_head_d',vars=dict(fromDate=from_dt2,toDate=to_dt2))) 

                        if btn_invoice_detail_d:
                            redirect (URL('invoice_detail_d',vars=dict(fromDate=from_dt2,toDate=to_dt2))) 
                
                        if btn_order_head_d:
                            redirect (URL('order_head_d',vars=dict(fromDate=from_dt2,toDate=to_dt2))) 

                        if btn_order_detail_d:
                            redirect (URL('order_detail_d',vars=dict(fromDate=from_dt2,toDate=to_dt2)))  
...
def daily_order_item_wise():
    c_id = session.cid

    response.title = 'Daily Order Item Wise '
    ...
    records = "SELECT depot_id,depot_name,client_id,client_name,store_id,store_name,market_id,market_name,rep_id,rep_name,sum(quantity) as orderCount,sum(quantity * price) as orderAmount,item_id,item_name,count(distinct(sl)) as sl  FROM sm_order WHERE cid = '" + c_id + "' AND order_date >= '" + str(startDt) + "' AND order_date <= '" + str(endDt) + "'"+condition+" GROUP BY  depot_id,client_id,rep_id,item_id;"
    records = db.executesql(records, as_dict=True)

    recordsSum = "SELECT sum(quantity) as orderCount,sum(quantity * price) as orderAmount,count(distinct(sl)) as sl  FROM sm_order WHERE cid = '" + c_id + "' AND order_date >= '" + str(startDt) + "' AND order_date <= '" + str(endDt) + "'"+condition+" GROUP BY cid;"
    recordsSum = db.executesql(recordsSum, as_dict=True)

    total_order=0
    
    recordsOrderCount = "SELECT sum(quantity * price) as orderAmount FROM sm_order WHERE cid = '" + c_id + "' AND order_date >= '" + str(startDt) + "' AND order_date <= '" + str(endDt) + "' AND status='Order' AND payment_mode='CASH' "+condition+" GROUP BY cid;"
    recordsOrderCount = db.executesql(recordsOrderCount, as_dict=True)
    # return recordsOrderCount
    total_order_count=0
    for i in range(len(recordsOrderCount)):
        recordsOrderCountStr = recordsOrderCount[i]
        orderAmount = recordsOrderCountStr['orderAmount']
        total_order_amount=orderAmount
        total_order+=total_order_amount

    
    recordsOrderAmount = "SELECT sum(quantity * price) as orderAmount FROM sm_order WHERE cid = '" + c_id + "' AND order_date >= '" + str(startDt) + "' AND order_date <= '" + str(endDt) + "' AND status='Order' AND payment_mode='CREDIT' " + condition + " GROUP BY cid;"
    recordsOrderAmount = db.executesql(recordsOrderAmount, as_dict=True)
    # return recordsOrderAmount
    total_order_amount = 0
    for i in range(len(recordsOrderAmount)):
        recordsOrderAmountStr = recordsOrderAmount[i]
        orderAmount = recordsOrderAmountStr['orderAmount']
        total_order_amount = orderAmount
        total_order += total_order_amount

    return dict(records=records,recordsSum=recordsSum,total_order_amount=total_order_amount,total_order_count=total_order_count,total_order=total_order, fromDate=fromDate, toDate=toDate, regionValue=regionValue,regionValueShow=regionValueShow, areaValue=areaValue, territoryValue=territoryValue,areaValueShow=areaValueShow, territoryValueShow=territoryValueShow, marketValue=marketValue,marketValueShow=marketValueShow, repCM=repCM, page=page, items_per_page=items_per_page)
...
def sales_research_level0():
    c_id=session.cid
    
    response.title='Sales Summary - '+session.level0Name+' wise'
...
def sales_research_level1():
    c_id=session.cid
    
    response.title='Sales Summary - '+session.level1Name+' wise'
...
def sales_research_rep():
    c_id=session.cid
    
    response.title='Sales Summary - SR wise'
...
def invoice_head_d():
    ...
    response.title = 'Invoice Head '

    from_dt = request.vars.fromDate

    to_dt = request.vars.toDate
    # return to_dt 
   ...
   # store_id,store_name,order_sl,
    records = "SELECT depot_id,depot_name,sl,order_datetime,delivery_date,client_id,client_name,rep_id,rep_name,level3_id,status,shipment_no,actual_total_tp,total_amount,discount,collection_amount,return_tp,return_discount,invoice_date,dist_discount FROM sm_invoice_head where cid = '" + c_id + "' AND invoice_date >= '" + str(from_dt)+ "' AND invoice_date <= '" + str(to_dt)+ "' order by invoice_date;"
    # return records
    records = db.executesql(records, as_dict=True)        

    myString = 'Invoice Head ' + '\n'    
    myString += 'From Date,' + str(from_dt) +' To '+str(to_dt)+ '\n'
    myString += '\n'

    myString += 'Depot Id,Depot Name,SL,Order DateTime, Delivery Date,Rep Id, Rep Name,Area,Status,Shipment No,Actual Total TP,Total Amount,Discount,Collection Amount,Return TP,Return Discount,Invoice Date,Dist Discount,Client Id,Client Name\n'
    ...
    for i in range(len(records)):
        ...
        depot_name = recStr['depot_name']
        sl = recStr['sl']
        order_datetime = recStr['order_datetime']
        delivery_date = recStr['delivery_date']
        client_id = recStr['client_id']
        client_name = recStr['client_name']
        rep_id = recStr['rep_id']
        rep_name = recStr['rep_name']
        level3_id = recStr['level3_id']
        status = recStr['status']
        shipment_no = recStr['shipment_no']
        actual_total_tp = recStr['actual_total_tp']
        total_amount = recStr['total_amount']
        discount = recStr['discount']
        collection_amount=recStr['collection_amount']
        return_tp=recStr['return_tp']
        return_discount = recStr['return_discount']
        invoice_date = recStr['invoice_date']
        dist_discount=recStr['dist_discount']
        
        myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_datetime) + ',' + str(
            delivery_date) + ',' + str(rep_id) + ',' + str(
            rep_name) + ',' + str(level3_id)+ ',' + str(status)+ ',' + str(shipment_no)+ ',' + str(actual_total_tp)+ ',' + str(total_amount)+ ',' + str(discount)+ ',' + str(collection_amount)+ ',' + str(return_tp) + ',' + str(return_discount)+ ',' + str(invoice_date)+ ',' + str(dist_discount)+ ',' + str(client_id) + ',' + str(client_name)  + '\n'
    ...
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_invoice_head.csv'
    return str(myString)



def invoice_detail_d():
    c_id = session.cid

    response.title = 'Invoice Details '
   ...
   # store_id,store_name,order_sl,
    records = "SELECT depot_id,depot_name,sl,order_datetime,delivery_date,client_id,client_name,rep_id,rep_name,level3_id,invoice_date,item_id,item_name,category_id,actual_tp,quantity,bonus_qty,price,return_qty,status,dist_discount_item FROM sm_invoice where cid = '" + c_id + "' AND invoice_date  >= '" + str(from_dt)+ "' AND invoice_date <= '" + str(to_dt)+ "' order by invoice_date;"
    # return records
    records = db.executesql(records, as_dict=True)        

    myString = 'Invoice Detail ' + '\n'    
    myString += 'From Date,' + str(from_dt) +' To '+str(to_dt)+ '\n'
    myString += '\n'

    myString += 'Depot Id,Depot Name,SL,Order DateTime, Delivery Date,Rep Id, Rep Name,Area,Invoice Date,Item ID, Item Name,Category,Actual TP,Quantity,Bonus Quantity, Price, Return Quantity,Status,Dist Discount Item,Client Id, Client Name\n'
    ...
    for i in range(len(records)):
        ...
        item_name = recStr['item_name']
        category_id = recStr['category_id']
        actual_tp=recStr['actual_tp']
        quantity=recStr['quantity']
        bonus_qty = recStr['bonus_qty']
        price = recStr['price']
        return_qty=recStr['return_qty']
        status = recStr['status']
        dist_discount_item=recStr['dist_discount_item']
       
        
        myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_datetime) + ',' + str(
            delivery_date)  + ',' + str(rep_id) + ',' + str(
            rep_name) + ',' + str(level3_id)+ ',' + str(invoice_date)+ ',' + str(item_id)+ ',' + str(item_name)+ ',' + str(category_id)+ ',' + str(actual_tp)+ ',' + str(quantity)+ ',' + str(bonus_qty) + ',' + str(price)+ ',' + str(return_qty)+ ',' + str(status) + ',' + str(dist_discount_item)+ ',' + str(client_id) + ',' + str(client_name) +'\n'
    ...
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_invoice_detail.csv'
    return str(myString)
...
def order_head_d():
    ...
    # store_id,store_name,order_sl,
    records = "SELECT sm_order_head.depot_id, sm_order_head.depot_name, sm_order_head.sl, sm_order_head.order_date, sm_order_head.delivery_date, sm_order_head.payment_mode, sm_order_head.client_id, sm_order_head.client_name, sm_order_head.rep_id, sm_order_head.rep_name, sm_order_head.level3_id, sm_order_head.level3_name, sm_order_head.status, sm_order_head.invoice_ref , sum(sm_order.quantity * sm_order.price) as total_value, sm_order.quantity  FROM sm_order_head INNER JOIN sm_order on sm_order_head.sl = sm_order.sl and sm_order_head.depot_id = sm_order.depot_id where sm_order_head.cid = '" + c_id + "' AND sm_order_head.order_date >= '" + str(from_dt)+ "' AND sm_order_head.order_date <= '" + str(to_dt)+ "' group by sm_order.sl ,sm_order.depot_id;"
    # return records
    records = db.executesql(records, as_dict=True)
    ...
    myString += 'Depot Id,Depot Name,SL,Order Date, Delivery Date,Payment Mode,Rep Id, Rep Name,AreaID,AreaName,Status,Invoice Ref,Order Amount,Client Id,Client Name\n'
    # return myString  
    for r in range(len(records)):
        ...
        depot_name = recStr['depot_name']
        sl = recStr['sl']
        order_date = recStr['order_date']
        delivery_date = recStr['delivery_date']
        payment_mode = recStr['payment_mode']
        client_id = recStr['client_id']
        client_name = recStr['client_name']
        rep_id = recStr['rep_id']
        rep_name = recStr['rep_name']
        ...
        level3_name = recStr['level3_name']
        status = recStr['status']
        invoice_ref = recStr['invoice_ref']
        total_value=recStr['total_value']

        myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_date) + ',' + str(delivery_date) + ',' + str(payment_mode) + ',' + str(rep_id) + ',' + str(rep_name) + ',' + str(level3_id) + ',' + str(level3_name) + ',' + str(status) + ',' + str(invoice_ref) + ','+str(total_value)+',' + str(client_id) + ',' + str(client_name) +'\n'
        # return myString
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_order_head.csv'
    return myString
...
def order_detail_d():
    ...
    # store_id,store_name,order_sl,
    records = "SELECT  sm_order.depot_id, sm_order.depot_name, sm_order.sl, sm_order.order_datetime, sm_order.delivery_date, sm_order.client_id, sm_order.client_name, sm_order.rep_id, sm_order.rep_name, sm_order.level3_id, sm_order.level3_name, sm_order.order_date, sm_order.item_id, sm_order.item_name, sm_order.category_id, sm_order.quantity, sm_order.price, sm_order.item_vat, sm_order.item_unit, sm_order.item_carton, sm_order.ho_status, sm_order.status, sm_depot_distributor.dist_disc_percent as discount FROM sm_order INNER JOIN sm_depot_distributor on sm_order.depot_id = sm_depot_distributor.depot_id  where sm_order.cid = '" + c_id + "' AND sm_order.order_date  >= '" + str(from_dt)+ "' AND sm_order.order_date <= '" + str(to_dt)+ "' order by sm_order.order_date;"
    # return records
    records = db.executesql(records, as_dict=True)        

    myString = 'Order Detail ' + '\n'    
    myString += 'From Date,' + str(from_dt) +' To '+str(to_dt)+ '\n'
    myString += '\n'

    myString += 'Depot Id,Depot Name,SL,Order Date, Delivery Date,Rep Id, Rep Name,AreaID, AreaName,Item ID, Item Name,Category,Item Carton,Item Unit,Quantity,Price,Order Amount, Distibutor Discount Percentage, Status,Client Id, Client Name \n'
    # total=0
    for recStr in records:
        total=0
        depot_id = recStr['depot_id']
        depot_name = recStr['depot_name']
        sl = recStr['sl']
        ...
        quantity = recStr['quantity']
        price = recStr['price']
        item_unit = recStr['item_unit']
        item_carton = recStr['item_carton']
        dist_discount_percentage = recStr['discount']
        status = recStr['status']
        # ho_status = recStr['ho_status']

        total+=quantity*price
        # return total

        
        # # Encode problematic strings as UTF-8
        # depot_id = depot_id.encode('utf-8')
        # depot_name = depot_name.encode('utf-8')
        # # order_date = order_date.encode('utf-8')
        # # delivery_date = delivery_date.encode('utf-8')
        # client_id = client_id.encode('utf-8')
        # client_name = client_name.encode('utf-8')
        # rep_id = rep_id.encode('utf-8')
        # rep_name = rep_name.encode('utf-8')
        # level3_id = level3_id.encode('utf-8')
        # level3_name = level3_name.encode('utf-8')
        # item_id = item_id.encode('utf-8')
        # item_name = item_name.encode('utf-8')
        # category_id = category_id.encode('utf-8')
        # # item_carton = item_carton.encode('utf-8')
        # item_unit = item_unit.encode('utf-8')
        # status = status.encode('utf-8')

        myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_date) + ',' + str(delivery_date) + ',' + str(rep_id) + ',' + str(rep_name) + ',' + str(level3_id) + ',' + str(level3_name) + ',' + str(item_id) + ',' + str(item_name) + ',' + str(category_id) + ',' + str(item_carton) + ',' + str(item_unit) + ',' + str(quantity) + ',' + str(price) + ',' + str(total) + ', '+str(dist_discount_percentage)+', '+str(status)+',' + str(client_id) + ',' + str(client_name)+'\n'
...
#    # store_id,store_name,order_sl,
#     records = "SELECT depot_id,depot_name,sl,order_date,delivery_date,payment_mode,client_id,client_name,rep_id,rep_name,level3_id,level3_name,status,invoice_ref FROM sm_order_head where cid = '" + c_id + "' AND order_date >= '" + str(from_dt)+ "' AND order_date <= '" + str(to_dt)+ "' order by order_date ;"
#     # return records
#     records = db.executesql(records, as_dict=True)        
...
#     myString += 'Depot Id,Depot Name,SL,Order Date, Delivery Date,Payment Mode,Rep Id, Rep Name,AreaID,AreaName,Status,Invoice Ref,Client Id,Client Name\n'
...
#         myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_date) + ',' + str(
#             delivery_date) + ',' + str(payment_mode)+ ',' + str(rep_id) + ',' + str(rep_name) + ',' + str(level3_id)+ ',' + str(level3_name)+ ',' + str(status)+ ',' + str(invoice_ref)+ ',' + str(client_id) + ',' + str(client_name)  + '\n'
...
#    # store_id,store_name,order_sl,
#     records = "SELECT depot_id,depot_name,sl,order_datetime,delivery_date,client_id,client_name,rep_id,rep_name,level3_id,level3_name,order_date,item_id,item_name,category_id,quantity,price,item_vat,item_unit,item_carton,ho_status FROM sm_order where cid = '" + c_id + "' AND order_date  >= '" + str(from_dt)+ "' AND order_date <= '" + str(to_dt)+ "' order by order_date;"
#     # return records
#     records = db.executesql(records, as_dict=True)        
...
#     myString += 'Depot Id,Depot Name,SL,Order Date, Delivery Date,Rep Id, Rep Name,AreaID, AreaName,Item ID, Item Name,Category,Item Carton,Item Unit,Quantity,Price,Client Id, Client Name\n'
...
#         myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_date) + ',' + str(delivery_date)  + ',' + str(rep_id) + ',' + str(rep_name) + ',' + str(level3_id)+ ',' + str(level3_name)+ ',' + str(item_id)+ ',' + str(item_name)+ ',' + str(category_id)+ ',' + str(item_carton)+ ',' + str(item_unit)+ ',' + str(quantity)+ ',' + str(price)+ ',' + str(client_id) + ',' + str(client_name) +'\n'
