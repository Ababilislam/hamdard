...
def base_mobile2():  
    # return 'abc'
    response.title='Base Mobile'
    submit=request.vars.submit
    delete = request.vars.btn_delete
    btn_filter = request.vars.btn_delete
    session.btn_filter=request.vars.btn_filter
    session.btn_all=request.vars.btn_all
    ...
    # return submit
    if submit:
        base_id=str(request.vars.base_id).strip().upper()
        session.base_id = base_id
        mobile_number=str(request.vars.mobile_number).strip()

            
        # return len(checkuserRows)
        # return userid
        if session.base_id!='' and mobile_number!='':
            # return userid
            check_userid_sql = "select base_id from base_mobile where base_id='"+str(session.base_id)+"';"
            checkuserRows = db.executesql(check_userid_sql, as_dict=True)
            # return len(checkuserRows)
            if len(checkuserRows) > 0:
                response.flash = 'Base id already exists'
                userRows_sql = "select base_id, mobile_number from base_mobile order by base_id;"
                # userRows = db.executesql(userRows_sql, as_dict=True)
                # return dict(userRows=userRows)


            else:
                insertOperator_sql = "INSERT INTO base_mobile (`cid`,`base_id`, `mobile_number`) VALUES ('"+str(c_id)+"','"+str(base_id)+"','"+str(mobile_number)+"');"
                # return insertOperator_sql
                # username=='OPERATOR':          
    ...
    if session.btn_filter:
        session.base_id=search_value.split('|')[0].upper()
        # session.base_id=base_id
        userRows_sql = "select base_id, mobile_number from base_mobile where base_id='"+str(session.base_id)+"' order by base_id;"
    else:
        
        userRows_sql = "select id,base_id, mobile_number from base_mobile order by base_id;"
    userRows = db.executesql(userRows_sql, as_dict=True)

    return dict(userRows=userRows)

def abc():
    response.title="Base Mobile Download"
    # return 'abc'
    # session.btn_filter=request.vars.btn_filter
    search_value=str(request.vars.searchValue_OpName).strip()
    # return search_value
    ...
    records = db.executesql(userRows_sql, as_dict=True)

    myString = 'Base Mobile List\n\n'
    myString += 'Base ID, Mobile Number \n'
    total=0
    attTime = ''
    totalCount = 0
    for i in range(len(records)):
        recordsStr = records[i]
        base_id = str(recordsStr['base_id'])
        mobile_number = str(recordsStr['mobile_number'])
        
      
        myString += str(base_id) + ','+str(mobile_number) +  ',\n'

    # Save as csv
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=Download_Base_Mobile_List.csv'
    return str(myString)
...
def user_list():
    ...
    for i in range(len(userRows)):
        ...
        base_id=str(records_ov_dict["base_id"])      
        mobile_number=str(records_ov_dict["mobile_number"])   
        if retStr == '':
            retStr = base_id+'|'+mobile_number
        else:
            retStr += ',' +base_id+'|'+mobile_number
...
def base_mobile_delete():  
    base_id=request.args[0]
    # return base_id
    mobile_number=str(request.vars.mobile_number).strip()
    delete_sql= "delete from base_mobile where base_id='"+str(base_id)+"'limit 1;"
    # return delete_sql
    delete = db.executesql(delete_sql)


    redirect(URL(c='settings',f='base_mobile2'))
    # return dict(delete)




 #=========================== BASE MOLIBE BATCH UPLOAD ===============================


def base_mobile_batch_upload():
   
    response.title = 'Base Mobile Batch Upload'
    ...
    if btn_upload=='Upload':        
        ...
        base_mobile_list_exist=[]   
        
        base_mobile_list_excel=[]
        ...
        for i in range(total_row):
            if i>=100:
                ...
                coloum_list=row_data.split( '\t')
                if len(coloum_list)==2:
                    baseMobileExcel=str(coloum_list[0]).strip().upper()
                    
                    if baseMobileExcel!='':
                        if baseMobileExcel not in base_mobile_list_excel:base_mobile_list_excel
                        base_mobile_list_excel.append(baseMobileExcel)
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)!=2:
                ...
                base_id_excel = str(coloum_list[0]).strip().upper()
                mobile_number = str(coloum_list[1]).strip().upper()
                # return mobile_number
                
                
                    
                if base_id_excel=='' or mobile_number=='':
                    # return mobile_number
                    error_data=row_data+'(Required all value)\n'
                    error_str=error_str+error_data
                    count_error+=1
                    ...
                    existCheckRows=db((db.base_mobile.base_id==base_id_excel)&(db.base_mobile.mobile_number==mobile_number)).select(db.base_mobile.base_id,limitby=(0,1))
                    # return existCheckRows
                    if existCheckRows:
                        error_data=row_data+'(Duplicate base_id/mobile_number/ check)\n'
                        error_str=error_str+error_data
                        count_error+=1
                        continue

                    else:
                        db.base_mobile.insert(base_id=base_id_excel,mobile_number=mobile_number)
                        # return mobile_number
                        count_inserted+=1
 ...
 #=========================== PROCESS ORDER ON OFF BUTTON FUNCTION ===============================#


def order_process():  
    response.title = "OrderProcess"
    cid = session.cid
    btn_on = str(request.vars.order_on_button).strip()
    btn_off = str(request.vars.order_off_button).strip()
    s_value = ''
    current_status = ''
    if btn_on =='Order ON':
        update_sql ="UPDATE sm_settings SET s_value = 'YES' where cid = '"+cid+"' and s_key='order_status' limit 1";
        # return update_sql
        update = db.executesql(update_sql)
        response.flash ="Order ON Successfully"
        # redirect(URL('utility_mrep', 'utility_settings'))

    elif btn_off =='Order OFF':
        update_sql ="UPDATE sm_settings SET s_value = 'NO' where cid = '"+cid+"' and s_key='order_status' limit 1";
        # return update_sql
        update = db.executesql(update_sql)
        response.flash ="Order OFF Successfully"
        # redirect(URL('utility_mrep', 'utility_settings'))

    current_order_status_sql="SELECT s_key, s_value from sm_settings where cid = '"+cid+"' and s_key='order_status' limit 1"
    # return current_order_status_sql
    current_order_status = db.executesql(current_order_status_sql, as_dict=True)
    for k in range(len(current_order_status)):
        records_ov_dict = current_order_status[k]
