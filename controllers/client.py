...
def client():
    ...
    if btn_filter_client:
        ...
        session.search_type_status=request.vars.search_type_status
        session.search_type_category=request.vars.search_type_category
        ...
        session.search_type_status = None
        session.search_type_category = None
        reqPage=0
    ...
    if session.btn_filter_client:
        ...
        if (session.search_type_status !=''): 
            search_type_status=str(session.search_type_status)
            qset=qset(db.sm_client.status==search_type_status)
            
        if (session.search_type_category != ''):
            search_type_category=str(session.search_type_category).split('|')[0]         
            qset=qset(db.sm_client.category_id==search_type_category)
            
    

    rows_check=db((db.sm_category_type.cid==cid) & (db.sm_category_type.type_name=='CLIENT_CATEGORY')).select(db.sm_category_type.cat_type_id, db.sm_category_type.cat_type_name)
    #return db._lastsql
    ...
    return dict(rows_check=rows_check,records=records,totalCount=totalCount,page=page,items_per_page=items_per_page,access_permission=access_permission,access_permission_view=access_permission_view)
...
def client_clean():
    cid=session.cid
    clean_check=request.vars.clean_check
    clean_pass=request.vars.clean_pass
    btn_filter_client=request.vars.btn_filter        
        
    if ((clean_check=='YES') & (clean_pass=='Confirm') & (session.btn_filter_client=='Filter')):
        qset=db()
        ...
        records=qset.delete()
        session.flash='Cleaned Successfully '
        redirect (URL('client','client'))

    else:
        session.flash='Please filter first and confirm '
        redirect (URL('client','client'))
...
def client_validation(form):
    ...
    p_market=str(request.vars.p_market_id).strip().upper()

    # p_market_id=str(request.vars.p_market_id).strip().upper().split('|')[0]
    # p_market_name=str(request.vars.p_market_id).strip().upper().split('|')[1]
    try:
        p_market_id=str(request.vars.p_market_id).strip().upper().split('|')[0]
        p_market_name=str(request.vars.p_market_id).strip().upper().split('|')[1]
    except:
                        ...
                        if not rows_check3:
                            ...
                            if check_client:            
                                ...
                                if len(address)>200:
                                    ...
                                    form.vars.p_market_id=p_market_id
                                    form.vars.p_market_name=p_market_name
...
#     db.sm_client.thana_id.requires=IS_EMPTY_OR(IS_IN_DB(db(db.thana.cid == session.cid),db.thana.id,'%(district)s - %(name)s',orderby=db.thana.district|db.thana.name))
    ...
    db.sm_client.category_id.requires=IS_IN_DB(db((db.sm_category_type.cid == session.cid)&(db.sm_category_type.type_name=='CLIENT_CATEGORY')),db.sm_category_type.cat_type_id,'%(cat_type_id)s | %(cat_type_name)s',orderby=db.sm_category_type.cat_type_id)
    db.sm_client.sub_category_id.requires=IS_IN_DB(db((db.sm_category_type.cid == cid)&(db.sm_category_type.type_name=='CLIENT_SUB_CATEGORY')),db.sm_category_type.cat_type_id,'%(cat_type_id)s | %(cat_type_name)s',orderby=db.sm_category_type.cat_type_id)
    db.sm_client.district_id.requires = IS_EMPTY_OR(
    IS_IN_DB(
        db(db.district.cid == cid),
        db.district.district_id,
        '%(district_id)s | %(name)s',
        orderby=db.district.name
    )
)
    
    form =SQLFORM(db.sm_client,
                  fields=['client_id','name','area_id','depot_id','store_id','category_id','sub_category_id','depot_belt_name','owner_name','contact_no1','contact_no2','address','p_market_id','market_id','thana','district_id','drug_registration_num','nid','doctor','status'],
                  submit_button='Save'       
                  )
    ...
    # form.vars.p_market_id=str(p_market_id)+'|'+str(p_market_name)    
...
def client_edit_validation(form):
    ...
    try:
        district_id=str(request.vars.district_id).strip().upper().split('|')[0]
        district_name=str(request.vars.district_id).strip().upper().split('|')[1]
    except:
        district_id=""
        district_name=""
...
def client_edit():
    ...
    if not records_client :
        ...
        district_id=str(records_client[0].district_id)
        district_name=str(records_client[0].district)
        area_id=str(records_client[0].area_id)
       

        p_market_id=str(records_client[0].p_market_id)
        p_market_name=str(records_client[0].p_market_name)
    ...
    form =SQLFORM(db.sm_client,
                  record=record,
                  deletable=False,
                  fields=['name','area_id','depot_id','store_id','category_id','sub_category_id','depot_belt_name','owner_name','contact_no1','contact_no2','address','p_market_id','market_id','thana','district_id','drug_registration_num','nid','doctor','status'],
    ...
    form.vars.p_market_id=p_market_id+'|'+p_market_name
    form.vars.district_id=district_id+'|'+district_name
...
def download_client():
    ...
    if (session.btn_filter_client):
        ...
        session.flash='Need filter by '+str(session.level0Name)+'/District/Market/Client/Status'
        redirect(URL('client'))
    ...
    myString+='Client New ID,Client Old ID,Name,Territory ID,Territory Name,Branch/Depot ID, Branch/Depot Name,Store ID,Store Name,Category ID,Category Name,Sub Category,Sub Category Name,Branch Belt,Contact Name,Contact1,Contact2,Address,Market ID,Market Name, Pocket Market ID, Pokcet Market Name, Thana,District ID,District Name,Drug Registration Number,NID,Doctor,Status,\n'
    for rec in records:
        client_id=str(rec.sm_client.client_id).replace(',', ' ').replace('\n', ' ')
        client_old_id=str(rec.sm_client.client_old_id).replace(',', ' ').replace('\n', ' ')
        name=str(rec.sm_client.name).replace(',', ' ').replace('\n', ' ')
        area_id=str(rec.sm_client.area_id).replace(',', ' ').replace('\n', ' ')
        depot_id=str(rec.sm_client.depot_id).replace(',', ' ').replace('\n', ' ')
        depot_name=str(rec.sm_client.depot_name).replace(',', ' ').replace('\n', ' ')
        level_name=str(rec.sm_level.level_name).replace(',', ' ').replace('\n', ' ')
        level0=str(rec.sm_level.level0).replace(',', ' ').replace('\n', ' ')  
        
        store_id=str(rec.sm_client.store_id).replace(',', ' ').replace('\n', ' ')
        store_name=str(rec.sm_client.store_name).replace(',', ' ').replace('\n', ' ')
              
        category_id=str(rec.sm_client.category_id).replace(',', ' ').replace('\n', ' ')
        category_name=str(rec.sm_client.category_name).replace(',', ' ').replace('\n', ' ')
        sub_category_id=str(rec.sm_client.sub_category_id).replace(',', ' ').replace('\n', ' ')
        sub_category_name=str(rec.sm_client.sub_category_name).replace(',', ' ').replace('\n', ' ')
        ...
        market_id=str(rec.sm_client.market_id).replace(',', ' ').replace('\n', ' ')
        market_name=str(rec.sm_client.market_name).replace(',', ' ').replace('\n', ' ')

        p_market_id=str(rec.sm_client.p_market_id).replace(',', ' ').replace('\n', ' ')
        p_market_name=str(rec.sm_client.p_market_name).replace(',', ' ').replace('\n', ' ')
             
        thana=str(rec.sm_client.thana).replace(',', ' ').replace('\n', ' ')
        district_id=str(rec.sm_client.district_id).replace(',', ' ').replace('\n', ' ')
        district=str(rec.sm_client.district).replace(',', ' ').replace('\n', ' ')
        nid=str(rec.sm_client.nid).replace(',', ' ').replace('\n', ' ')
        drug_registration_num=str(rec.sm_client.drug_registration_num).replace(',', ' ').replace('\n', ' ')
        doctor=str(rec.sm_client.doctor).replace(',', ' ').replace('\n', ' ')
        status=str(rec.sm_client.status).replace(',', ' ').replace('\n', ' ')
...
#         +level_name+','+level0+','
        ...
        depot_belt_name+','+owner_name+','+contact_no1+','+contact_no2+','+address+','+market_id+','+market_name+','+p_market_id+','+p_market_name+','+thana+','+district_id+','+district+','+drug_registration_num+','+nid+','+doctor+','+status+'\n'
...
def get_p_market_list():
    rep1Str = ''
    cid = session.cid
    areaId=request.vars.areaId

    rows = db((db.sm_microunion.cid == cid) &(db.sm_microunion.area_id == areaId) ).select(db.sm_microunion.microunion_id, db.sm_microunion.microunion_name, orderby=db.sm_microunion.microunion_name)
    ...
    for row in rows:
        ...
        if rep1Str == '':
            rep1Str = microunion_id + '|' + microunion_name 
        else:
            rep1Str += ',' + microunion_id + '|' + microunion_name 
...
def client_status_active():
    response.title = 'Client Status Active Inactive'
    client_id_str=''
    cid = session.cid
    client_id_str==''
    client_id = str(request.vars.client_id).strip()
    # return client_id
    active_button = request.vars.btn_active
    inactive_button = request.vars.btn_inactive
    # client_id = client_id.replace(' ','')
    client_id = client_id.split('\n')

    for i in range(len(client_id)):
        clientID=str(client_id[i]).strip()
        if clientID!='':
            if client_id_str=='':
    ...
    if active_button:
        if client_id == '' or client_id == 'None' or client_id == 'NONE' or client_id == None:
            response.flash = 'Please Enter valid Client ID'
            redirect(URL('client','client_status_active'))
        else:
            update_client_status_active_sql = "UPDATE sm_client set status = 'ACTIVE' WHERE cid = '"+str(cid)+"' and client_id in ("+client_id_str+") " 
            # return update_client_status_active_sql
            update_client_status_active = db.executesql(update_client_status_active_sql)
            response.flash = 'Client ACTIVE Successfully'
    elif inactive_button:
        if client_id == '' or client_id == 'None' or client_id == 'NONE' or client_id == None:
            response.flash = 'Please Enter valid Client ID'
            redirect(URL('client','client_status_active'))
        else:
            update_client_status_inactive_sql = "UPDATE sm_client set status = 'INACTIVE' WHERE cid = '"+str(cid)+"' and client_id in ("+client_id_str+")" 
            # return update_client_status_inactive_sql
            update_client_status_inactive = db.executesql(update_client_status_inactive_sql)
            response.flash = 'Client INACTIVE Successfully'
...
def get_district_list():
    ...
    query = f"select district_id,name from district;"
    rows = db.executesql(query,as_dict=True)
    for i in range(len(rows)):
        item_list_dict=rows[i]
        level_id = str(item_list_dict["district_id"])
        name = str(item_list_dict["name"])

        if dresStr == '':
            dresStr = level_id + '|' + name
        else:
            dresStr += ',' + level_id + '|' + name
