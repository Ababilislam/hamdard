...
def slsRptDcr_area_wise():
    ...
    if (user_type=='REP'):
        ...
        sql_str="SELECT  COUNT(s1.doc_id)as doc_count,s1.level3_id as route_id,route_name,s1.doc_id as doc_id, s1.doc_name as doc_name,s1.id as id,s1.visit_dtime as visit_dtime,s1.giftnsample as giftnsample  FROM sm_doctor_visit as s1  WHERE s1.cid = '"+ str(cid) +"' AND s1.rep_id = '"+ str(rep_id) +"' "+ condition + " GROUP BY s1.route_id,s1.id;"
        # return sql_str
        records_ov=db.executesql(sql_str,as_dict = True)
    ...
    if (user_type=='SUP'):
        ...
        records_ov=[]
        if int(depth)==0:
            sql_str="SELECT  COUNT(dv.doc_id)as doc_count,dv.doc_id as doc_id, dv.doc_name as doc_name,dv.id as id,dv.visit_dtime as visit_dtime,dv.giftnsample as giftnsample,dv.level3_id as route_id,level3_name FROM sm_doctor_visit as dv  WHERE dv.cid = '"+ str(cid) +"'  AND dv.visit_date >= '"+ str(from_dt) +"' AND dv.visit_date <= '"+ str(to_date) +"'  AND dv.level2_id = '"+ str(route_id) +"' GROUP BY dv.id"
