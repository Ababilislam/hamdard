...
def item():
    ...
    form =SQLFORM(db.sm_item,
                  fields=['item_id','name','des','category_id','category_id_sp','unit_type','manufacturer','item_carton','item_per_unit','price','vat_amt'],       
                  submit_button='Save'
                  )
...
def item_edit():
    ...
    form =SQLFORM(db.sm_item,
                  ...
                  linkto=None,
                  upload=None,
                  fields=['name','des','category_id','category_id_sp','unit_type','manufacturer','item_carton','item_per_unit','price','vat_amt','status'],
    ...
    if btn_upload=='Upload':        
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)==12:
                ...
                item_per_unit = str(coloum_list[10]).strip()
                status=str(coloum_list[11]).strip().upper()
                                        ...
                                        if duplicate_item==True:
                                            ...
                                            if price<0 or vat_amt<0:
                                                ...
                                                if (itemListStr.find(temp_item)== (-1)):
                                                    ...
                                                    ins_dict= {'cid':c_id,'item_id':item_id,'name':name,'des':des,'category_id':category_id,'category_id_sp':category_id_sp,'unit_type':unit_type,'manufacturer':manufacturer,'item_carton':mCarton,'price':price,'vat_amt':vat_amt,'total_amt':total_amt, 'item_per_unit':item_per_unit,'status':status}
...
def download_item():
    ...
    myString='Item List\n'
    myString+='Item ID,Name,Description,Group/Category,Base Group/Category,Unit Type,Manufacturer,M.Carton,Item Per Unit,TP Amount,VAT Amount,Total Amount,Status\n'
    ...
    for rec in records:
        ...
        item_per_unit=rec.item_per_unit
        price=rec.price
        vat_amt=rec.vat_amt        
        ...
        if category_id_sp==None:
            category_id_sp=''
        myString+=str(item_id)+','+str(name)+','+str(des)+','+str(category_id)+','+str(category_id_sp)+','+str(unit_type)+','+str(manufacturer)+','+str(item_carton)+','+str(item_per_unit)+','+str(price)+','+str(vat_amt)+','+str(total_amt)+','+str(status)+'\n'
...
#=================================== ITEM EDIT BATCH UPLOAD ===============================#

def item_edit_batch_upload():
   
    response.title = 'Item Edit Batch Upload'
    cid=session.cid
    btn_upload=request.vars.btn_upload
    count_inserted=0
    count_error=0
    ...
    if btn_upload=='Upload':
        ...
        for i in range(total_row):
            ...
            if len(coloum_list)!=3:
                ...
                item_id_excel = str(coloum_list[0]).strip()
                # return item_id_excel
                price_excel = str(coloum_list[1]).strip()
                total_qty_excel = str(coloum_list[2]).strip()                      
                    
                if item_id_excel=='' or item_id_excel == 'None' or item_id_excel == None or item_id_excel== 'NONE' or price_excel == '' or price_excel == 'None' or price_excel == None or total_qty_excel == '' or total_qty_excel == 'None' or total_qty_excel == None :
                    error_data=row_data+'(Required all value)\n'
                    error_str=error_str+error_data
                    count_error+=1
                    continue                    
                    ...
                    existCheckRows= " select * FROM sm_item WHERE cid='"+str(cid)+"' and item_id = '"+str(item_id_excel)+"' LIMIT 1"
                    # return existCheckRows
                    existCheck = db.executesql(existCheckRows)

                    if len(existCheck) > 0:
                        update_item_sql = "UPDATE sm_item SET price = '"+str(price_excel)+"', total_amt = '"+str(total_qty_excel)+"' WHERE cid ='"+str(cid)+"' and item_id = '"+str(item_id_excel)+"' limit 1 ; "
                        # return update_item_sql
                        update_item_record = db.executesql(update_item_sql)
                        count_inserted+=1

                    else:
                        error_data=row_data+'(Item is not Valid)\n'
                        error_str=error_str+error_data
                        count_error+=1
                        continue
