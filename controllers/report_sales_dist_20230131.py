...
def inv_preview_dist():
    ...
    if distRow:
        ...
        if hRecords:
            ...
            records = db((db.sm_invoice.cid == c_id) & (db.sm_invoice.depot_id == depot_id) & (db.sm_invoice.invoice_date >= fromDate)& (db.sm_invoice.invoice_date <= toDate)).select(db.sm_invoice.item_id,db.sm_invoice.item_name,db.sm_invoice.batch_id,db.sm_invoice.category_id,db.sm_invoice.actual_tp,db.sm_invoice.actual_vat,db.sm_invoice.price,db.sm_invoice.item_vat,db.sm_invoice.short_note,db.sm_invoice.item_unit,db.sm_invoice.promotion_type,db.sm_invoice.bonus_applied_on_qty,db.sm_invoice.discount_type,db.sm_invoice.item_discount,db.sm_invoice.discount_type_quantity,db.sm_invoice.quantity.sum(),db.sm_invoice.bonus_qty.sum(),groupby=db.sm_invoice.item_id, orderby=db.sm_invoice.item_name)
        ...
        # -------------------------
    return dict(data_List=data_List, dist_id=dist_id, dist_name=dist_name,fromDate=fromDate,toDate=toDate)
...
def invoice_item_list_synopsis_sd_after_del_dist():
    ...
    return dict(records=records, maxMarketID=maxMarketID, maxmarketName=maxmarketName, paymentMode=paymentMode,
                ...
                return_discount=return_discount,dist_id=dist_id,dist_name=dist_name,depot_id=depot_id,depot_name=depot_name,dist_disc_percent=dist_disc_percent,fromDate=fromDate,toDate=toDate)
