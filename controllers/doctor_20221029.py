...
def download_doctor():
    ...
    for rec in records:
        ...
        docName = str(rec.doc_name).replace(',', ' ').replace('\n', ' ')
        specialty = str(rec.specialty).replace(',', '; ').replace('\n', ' ')
        degree = str(rec.field1).replace(',', '; ').replace('\n', ' ')
        chamberAddress = str(rec.des).replace(',', '; ').replace('\n', ' ')
        attachInstitute = str(rec.attached_institution).replace(',', '; ').replace('\n', ' ')
        designation = str(rec.designation).replace(',', '; ').replace('\n', ' ')
        dob = str(rec.dob).replace(',', ' ').replace('\n', ' ')          
        mobile = str(rec.mobile).replace(',', ' ').replace('\n', ' ')
        doctorsCategory = str(rec.doctors_category).replace(',', '; ').replace('\n', ' ')
        status = str(rec.status).replace(',', ' ').replace('\n', ' ')
        updated_by = str(rec.updated_by).replace(',', ' ').replace('\n', ' ')
...
def microunion():
    ...
    microunion_area_get=request.vars.microunion_area
    # return microunion_area_get
    # return microunion_area_get.split('|')[1]   
    try: 
        form.vars.area_id=microunion_area_get.split('|')[0]
        form.vars.area_name=microunion_area_get.split('|')[1]   
        # return microunion_area_get.split('|')[1]
    except:
        pass
    ...
    return dict(form=form,records=records,recordsCount=recordsCount,page=page,items_per_page=items_per_page,access_permission=access_permission)
