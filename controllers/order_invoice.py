...
#             qset=qset(db.sm_invoice_head.depot_id==session.depot_id)
    ...
    if session.btn_filter_invoice:
        ...
        if session.search_ord_date_from!=None and session.search_ord_date_to!=None:
            ...
            qset=qset((db.sm_invoice_head.invoice_date>=search_ord_date_from_v)&(db.sm_invoice_head.invoice_date<search_ord_date_to_v))
...
#     
    ...
    # return session.discount_precent    
    ...
    records=db((db.sm_invoice.cid==c_id)& (db.sm_invoice.depot_id==depot_id) & (db.sm_invoice.sl==req_sl) & (db.sm_invoice.status!='Invoiced_99')).select(db.sm_invoice.ALL,orderby=db.sm_invoice.item_name)
    storeRecords=''
    if sl<=0:
    ...
    # ========================================
        orderRows=db((db.sm_invoice.cid==c_id)& (db.sm_invoice.depot_id==depot_id) & (db.sm_invoice.sl==req_sl)).select(db.sm_invoice.ALL,orderby=db.sm_invoice.item_name)
        # return orderRows
        if orderRows:
            discount_precent=0
            category_id=''
            for ordRow in orderRows:
                ...
                # batch_id=ordRow.batch_id
                # depot_id=ordRow.depot_id
                # sl=ordRow.sl
                # item_id=ordRow.item_id
                # if batch_id == '':
                #     update_batch_id_sql = "Update sm_invoice set batch_id = '1001' where depot_id = '"+str(depot_id)+"' and item_id = '"+str(item_id)+"'"
                #     update_batch_id = db.executesql(update_batch_id_sql)
        ...
        # =========================================
        discount_precent=session.discount_precent
...
def update_invoice_item():
    ...
    if btn_delete:
        ...
        if not autDelCronRows:
            ...
            countRecords=db((db.sm_invoice.cid==c_id)& (db.sm_invoice.depot_id==req_depot) & (db.sm_invoice.sl==req_sl) & (db.sm_invoice.quantity>0)).count()
    ...
    #------------------ INVOCIE reprocess/promotional rules apply
        ...
        if not autDelCronRows:
            ...
            if not headRecords:
                ...
                retRes=get_order_to_delivery_detail_rules_manual_for_reprocess(c_id,depot_id,invoice_sl,order_sl,clientId,orderDate)
...
def get_order_to_delivery_detail_rules_manual_for_reprocess(cid,depot_id,invoice_sl,order_sl,clientId,orderDate):
    c_id=cid
    depot_id=depot_id
    sl=order_sl
    client_id=clientId
    ...
    if session.discount_precent=='' or session.discount_precent==None or session.discount_precent=='None' or session.discount_precent=='NONE':
        session.discount_precent=0
