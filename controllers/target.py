...
def item_batch_validation(form):    
    ...
    # territory_id=str(request.vars.territory_id).strip().upper().split('|')[0]
...
def target_add_route_item():
    ...
    form.vars.territory_id=territory_id
    form.vars.item_id=item_id
    # return territory_id
    ...
    if form.accepts(request.vars, session, 
        onvalidation=validation_target_route_item):
    ...
    if btn_filter_target:
        ...
        session.target_route_id_name=request.vars.target_route_id_name

        reqPage=0

    elif btn_all:
        session.btn_filter_target=None
        session.target_route_id_name=None
    ...
    if (session.btn_filter_target!=None and session.target_route_id_name!=None and session.target_route_id_name!=''):
        target_route_id_name=str(session.target_route_id_name).split('|')[0]          
        qset=qset(db.target_vs_achievement_route_item.territory_id==target_route_id_name)    

    records = qset.select(db.target_vs_achievement_route_item.ALL,orderby=~db.target_vs_achievement_route_item.first_date | db.target_vs_achievement_route_item.territory_id | db.target_vs_achievement_route_item.item_id,limitby=limitby)
    # return records
...
def target_add_route_item_edit():
    ...
    page= request.args(0)
    record= db.target_vs_achievement_route_item(request.args(1)) #or redirect(URL('index'))
    ...
    form =SQLFORM(db.target_vs_achievement_route_item,
                  record=record,
                  deletable=True,
                  fields=['territory_id','item_id','item_name','target_qty'],
                  submit_button='Update'
                  )
    
    # records_rep=db((db.target_vs_achievement_route_item.cid==session.cid) & (db.target_vs_achievement_route_item.id==request.args(1))).select(db.target_vs_achievement_route_item.territory_id,limitby=(0,1))
    # territory_id=''
    # for records_show_id in records_rep :
    #      territory_id=records_show_id.territory_id
    #      break
    
    if form.accepts(request.vars, session):#,onvalidation=rep_edit_validation):
        # rep_name=form.vars.name
        # db((db.target_vs_achievement_route_item_area.cid==session.cid) & (db.target_vs_achievement_route_item_area.territory_id==territory_id)).update(rep_name=rep_name)
        
        session.flash = 'Update Successfully'
        redirect(URL('target_add_route_item',args=[page]))
    
    #If rep exist in rep area useflag will true
    # useFlag=False
    # repareaRows=db((db.target_vs_achievement_route_item_area.cid==session.cid) & (db.target_vs_achievement_route_item_area.rep_id==rep_id)).select(db.target_vs_achievement_route_item_area.rep_id,orderby=db.target_vs_achievement_route_item_area.rep_id,limitby=(0,1))
    # if repareaRows:
    #     useFlag=True
    #------------------
    
    return dict(form=form,page=page, territory_id=territory_id)
...
def target_batch_upload_route_item():
    ...
    btn_clean_product = request.vars.btn_clean_product
...
#================================== PRODUCT CLEAN =====================================#
    if btn_clean_product:
        year = request.vars.clean_year_product
        month = request.vars.clean_month_product
        product_id = str(request.vars.clean_product).upper().split('-')[0]
        # return year, month,product_id
        if year == '' or month == '':
            response.flash = 'Valid Year-Month required'
        else:
            if product_id=='':
                response.flash = 'Valid Product required'
            else:
                first_date = year + '-' + month + '-01'
                # return first_date
                try:
                    product_id = str(request.vars.clean_product).split('|')[0]
                except:
                    product_id = str(request.vars.clean_product).upper().split('-')[0]

                db((db.target_vs_achievement_route_item.cid == c_id) & (db.target_vs_achievement_route_item.first_date == first_date) & (db.target_vs_achievement_route_item.item_id == product_id)).delete()
                # return db._lastsql
                response.flash = 'Successfully Updated'
                    ...
                    if (valid_route == False):
                        ...
                        if valid_item == False:
                            ...
                            if existRow:
                                ...
                                try:
                                    db.target_vs_achievement_route_item.insert(cid=c_id, first_date=firstDate, target_date=date_ex, item_id=item_id_ex, item_name=itemName, price=price, category_id=category_id, zone_id=level0_id, zone_name = level0_name, region_id=level1_id, region_name=level1_name, area_id=level2_id, area_name=level2_name, territory_id=routeID_ex, territory_name=level3_name, depot_id=route_depotId, target_qty=targetqty)
                                    count_inserted += 1
                                except:
...
def get_target_route_id_name():

    targetRoute = ''
    cid = session.cid        
    rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 3)).select(db.sm_level.level3, db.sm_level.level3_name, orderby=db.sm_level.level3_name, groupby=db.sm_level.level3_name)

    for row in rows:
        level3 = str(row.level3).replace('|', ' ').replace(',', ' ')        
        level3_name = str(row.level3_name).replace('|', ' ').replace(',', ' ')
        if targetRoute == '':
            targetRoute = level3 + '|' + level3_name            
        else:
            targetRoute += ',' + level3 + '|' + level3_name
    return targetRoute


def get_target_route_id_name_input():

    targetRouteInput = ''
    cid = session.cid        
    # rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 3)).select(db.sm_level.level3, orderby=db.sm_level.level3, groupby=db.sm_level.level3)
    rows = db((db.sm_level.cid == cid)&(db.sm_level.depth == 3)).select(db.sm_level.level3, db.sm_level.level3_name, orderby=db.sm_level.level3_name, groupby=db.sm_level.level3_name)
    ...
    for row in rows:
        ...
        level3_name = str(row.level3_name).replace('|', ' ').replace(',', ' ')
        if targetRouteInput == '':
            targetRouteInput = level3 + '|' + level3_name            
        else:
            targetRouteInput += ',' + level3 + '|' + level3_name
    return targetRouteInput

def get_target_item_id_name():

    targetItem = ''
    cid = session.cid        
    rows = db((db.sm_item.cid == cid)).select(db.sm_item.item_id, db.sm_item.name, orderby=db.sm_item.name, groupby=db.sm_item.name)

    for row in rows:
        item_id = str(row.item_id).replace('|', ' ').replace(',', ' ')        
        name = str(row.name).replace('|', ' ').replace(',', ' ')
        if targetItem == '':
            targetItem = item_id + '|' + name            
        else:
            targetItem += ',' + item_id + '|' + name
    return targetItem

def get_target_item_id_name_name():

    targetItemName = ''
    cid = session.cid        
    rows = db((db.sm_item.cid == cid)).select(db.sm_item.name, orderby=db.sm_item.name, groupby=db.sm_item.name)
...
def get_active_item_id_name():

    targetItem = ''
    cid = session.cid        
    rows = db((db.sm_item.cid == cid) & (db.sm_item.status == 'ACTIVE')).select(db.sm_item.item_id, db.sm_item.name, orderby=db.sm_item.name, groupby=db.sm_item.name)
    # db._lastsql
    for row in rows:
        item_id = str(row.item_id).replace('|', ' ').replace(',', ' ')        
        name = str(row.name).replace('|', ' ').replace(',', ' ')
