...
#         return marketTourRows1
        ...
        # return session.userdepth
        if int(session.userdepth)>1:
...
#             return marketTourRows
        elif int(session.userdepth)==1:
            marketTourRows=db((db.sm_level.cid == cid)& (db.sm_level.depth == '2') & (db.sm_level[supLevel].belongs(areaList)) ).select(db.sm_level.level_id,db.sm_level.level_name,orderby=db.sm_level.level_id)
            # return marketTourRows
        else:
...
def tourSave():
    ...
    if not repRow:
       ...
       return retStatus
        ...
        planCheck = db((db.sm_doctor_visit_plan.cid == cid) & (db.sm_doctor_visit_plan.rep_id == rep_id) & (db.sm_doctor_visit_plan.first_date == first_date) ).select(db.sm_doctor_visit_plan.rep_id,db.sm_doctor_visit_plan.rep_name)
        return planCheck
        if not planCheck:
            msg='Plan your tour first'
...
def tourReject():
    ...
    if Month_get=='Next':
        ...
        thisM_fdate=str(nextMaont).split('-')[0]+'-'+str(nextMaont).split('-')[1]+'-01'
    # return thisM_fdate
    tourThisEvening = db((db.sm_doctor_visit_plan.rep_id == rep_id) & (db.sm_doctor_visit_plan.first_date == thisM_fdate)).update(status='Draft')
...
def tourPending():
    response.title='Amendment'
    rep_id = request.args[0].upper()
    ...
    pendinginfo = db((db.sm_doctor_visit_plan.cid==cid) &(db.sm_doctor_visit_plan.rep_id==rep_id)  & (db.sm_doctor_visit_plan.status=='CReq') & (db.sm_doctor_visit_plan.schedule_date > s_date) ).select(db.sm_doctor_visit_plan.schedule_date,db.sm_doctor_visit_plan.route_id,db.sm_doctor_visit_plan.route_name,db.sm_doctor_visit_plan.note,db.sm_doctor_visit_plan.status, orderby=~db.sm_doctor_visit_plan.note, groupby=db.sm_doctor_visit_plan.schedule_date)    
    # return db._lastsql
 
 
    return dict(pendinginfo=pendinginfo,s_date=s_date,rep_id=rep_id,cid=cid)  
...
def amndApprove():
    ...
    deletePPlan = db((db.sm_doctor_visit_plan.cid==cid)  &(db.sm_doctor_visit_plan.rep_id==rep_id) & (db.sm_doctor_visit_plan.status=='Confirmed') & (db.sm_doctor_visit_plan.schedule_date== s_date)).delete()
    updatePlan = db((db.sm_doctor_visit_plan.cid==cid)  &(db.sm_doctor_visit_plan.rep_id==rep_id) & (db.sm_doctor_visit_plan.status=='CReq') &(db.sm_doctor_visit_plan.schedule_date== s_date)).update(status='Confirmed')
      
    if updatePlan:
        session.flash="Submitted Approved"
    redirect(URL(c='tour_web',f='tourPending',args=[rep_id,cid]))
    # return dict(rep_id=rep_id)  
...
def amndReject():
    ...
    updatePlan = db((db.sm_doctor_visit_plan.cid==cid)  &(db.sm_doctor_visit_plan.rep_id==rep_id) & (db.sm_doctor_visit_plan.status=='CReq') &(db.sm_doctor_visit_plan.schedule_date== s_date)).update(status='Cancelled')
     
    if updatePlan:
        session.flash="Submitted Rejected"
        pass
    redirect(URL(c='tour_web',f='tourPending',args=[rep_id,cid]))   
...
#         return rep_name
        docListNew_S=urllib.parse.unquote(docListNew)
...
#         return rep_name
        clntListNew_S=urllib.parse.unquote(clntListNew)
