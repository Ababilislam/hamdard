...
def print_invoice():
    ...
    # return session.search_type_printm
                                ...
                                if not autDelCronRows:
                                    ...
                                    for j in range(len(slList)):
                                        ...
                                        if not headRow:
                                            ...
                                            discount_precent = headRow[0].discount_precent
                                            ...
                                            # inv discount chk
                                            if discount == 0:
                                                errorStr = 'Check Discount of Invoice SL' + str(req_sl)
                                            else:
                                                # bonus item check
                                                bonusItemCount = 0;
                                                bonusItemBonusCount = 0;
                                                bonusRows = db((db.sm_invoice.cid == c_id) & (db.sm_invoice.depot_id == req_depot) & (db.sm_invoice.sl == req_sl) & (db.sm_invoice.status == 'Submitted')).select(db.sm_invoice.promotion_type, db.sm_invoice.bonus_qty)
                                                for brow in bonusRows:
                                                    promotion_type = str(brow.promotion_type).upper()
                                                    ...
                                                    if int(bonus_qty) > 0:
                                                        bonusItemBonusCount += 1

                                                if bonusItemCount > bonusItemBonusCount:
                                                    errorStr = 'Check Bonus Item of Invoice SL' + str(req_sl)
                                                else:
                                                    itemStrForQty=''
...
def invoice_list_preview():
    ...
    vslListStr=urllib.parse.unquote(vslListStr)
    ...
    for i in range(len(vslList)):
        ...
        if hRecords:        
            ...
            for vdRow in records:        
                ...
                # item_id = str(item_id)
                # return item_id,fmcg_item
                # return fmcg_item.find(item_id)
                # if fmcg_item.find(item_id) == -1:
                #     flag = 1
                    # break
    ...
    sup_id=''
    sup_name = ''
    supRows = db((db.sm_supervisor_level.cid == session.cid) & (db.sm_supervisor_level.level_id == level2_id) & (db.sm_supervisor_level.level_depth_no == 2)).select(db.sm_supervisor_level.sup_id,db.sm_supervisor_level.sup_name, limitby=(0, 1))
    if supRows:
        sup_id = supRows[0].sup_id
        sup_name = supRows[0].sup_name

        #-------------------------

    # return data_List
    # return flag
    return dict(data_List=data_List,dist_name=dist_name,sup_id=sup_id,sup_name=sup_name, fmcg_item = fmcg_item, flag =  flag)
...
def print_synopsis():
    ...
    from_date_delivery=str(request.vars.from_date_delivery).strip()
    to_date_delivery =str(request.vars.to_date_delivery ).strip()
    ...
    if btn_filter_printm_s:
        ...
        # ==================
         #---
        try:
            currentDateFrom_d=datetime.datetime.strptime(from_date_delivery,'%Y-%m-%d').strftime('%Y-%m-%d')
            session.currentDateFrom_d=currentDateFrom_d
        except:
            session.currentDateFrom_d=''
            
        #---
        try:
            currentDateTo_d=datetime.datetime.strptime(to_date_delivery,'%Y-%m-%d').strftime('%Y-%m-%d')
            session.currentDateTo_d=to_date_delivery
        except:
            session.currentDateTo_d=''
    ...
    if (session.user_type!='Depot'):
        ...
        if (session.btn_filter_printm_s):
            ...
            if session.currentDateFrom_d!='' and session.currentDateTo_d!='':
                qset=qset((db.sm_invoice_head.delivery_date>=session.currentDateFrom_d)&(db.sm_invoice_head.delivery_date<=session.currentDateTo_d))
...
def invoice_item_list_synopsis_sd_after_del():
    ...
    if (session.btn_filter_printm_s):
        ...
        if session.currentDateFrom_d!='' and session.currentDateTo_d!='':
            qset=qset((db.sm_invoice_head.delivery_date>=session.currentDateFrom_d)&(db.sm_invoice_head.delivery_date<=session.currentDateTo_d))
            del_dateFrom=session.currentDateFrom_d
            del_dateTo=session.currentDateTo_d
    ...
    area_name_list = []
    rep_name_list = []
    headRecords2=qset.select(db.sm_invoice_head.market_id,db.sm_invoice_head.market_name.max(),db.sm_invoice_head.id.count(),db.sm_invoice_head.rep_id,db.sm_invoice_head.rep_name,db.sm_invoice_head.area_id,db.sm_invoice_head.area_name,groupby=[db.sm_invoice_head.area_id, db.sm_invoice_head.rep_id])
    # return db._lastsql
    for headRec2 in headRecords2:
        market_id=headRec2.sm_invoice_head.market_id
        market_name=headRec2[db.sm_invoice_head.market_name.max()]
    ...
    #-------------------------
    rep_name_list = str(rep_name_list).split(',')
    rep_name_list = str(rep_name_list).replace('[', '').replace(']','').replace('"','').replace("'",'')
    # return rep_name_list

    area_name_list = str(area_name_list).split(',')
    area_name_list = str(area_name_list).replace('[', '').replace(']','').replace('"','').replace("'",'')
    # return area_name_list

    return dict(records=records,rep_name_list =rep_name_list, area_name_list=area_name_list, maxMarketID=maxMarketID,maxmarketName=maxmarketName,paymentMode=paymentMode,creditType=creditType,search_from_sl=search_from_sl,search_to_sl=search_to_sl,territory_id=territory_id,territory_name=territory_name,invoice_dateFrom=invoice_dateFrom,invoice_dateTo=invoice_dateTo,d_man_id=d_man_id,d_man_name=d_man_name,rep_id=rep_id,rep_name=rep_name,area_id=area_id,area_name=area_name,clientCount=clientCount,invoiceTotal=invoiceTotal,actual_total_tp=actual_total_tp,return_sp_discount=return_sp_discount,totalAmount=totalAmount,vatTotal=vatTotal,totalDiscount=totalDiscount,totalSpDiscount=totalSpDiscount,return_tp=return_tp,return_vat=return_vat,return_discount=return_discount,del_dateFrom=del_dateFrom,del_dateTo=del_dateTo)
...
def invoice_item_list_synopsis_sd_after_del_download():
    ...
    myString+='Delivery From Date:,'+str(del_dateFrom)+'\n'
    myString+='Delivery To Date:,'+str(del_dateTo)+'\n'
...
def invoice_item_list_synopsis_ad_after_del_download():
    ...
    myString+='Cust ID,Customer Name,Terms,Invoice,Date,Order Amount,Tp,VAT,Disc,SpDisc,Net Sale'+'\n'
    for record in records:        
        client_id=record.client_id
        client_name=str(record.client_name).replace(',', ' ')        
        ...
        #------------------------        
        myString+=str(client_id)+','+str(client_name)+','+str(payment_mode)+','+str(invSl)+','+str(invoice_date)+','+str(ordAmt)+','+str(tpAmt)+','+str(vatAmt)+','+str(discAmt)+','+str(spDiscAmt)+','+str(totalAmt)+'\n'
