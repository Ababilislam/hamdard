...
def product_bonus_validation(form):    
    ...
    if dateFlag==False:
        ...
        if bonus_type =='':
            response.flash="Please Select Bonus Type"
        else:            
            pass
...
def product_bonus():
    ...
    form =SQLFORM(db.sm_promo_product_bonus,
                  fields=['from_date','to_date','min_qty','bonus_type','circular_number','note','allowed_credit_inv','regular_discount_apply','status'],       
                  submit_button='Save'
                  )
...
def product_bonus_download():
    ...
    myString+='Ref,Circular Number,From-date,To-date,Note,Min Qty,Bonus Type, Status\n'
    #Replace coma from records. cause coma means new Column    
    for rec in records:
        refrowid=str(rec.id)
        ...
        myString+=str(refrowid)+','+str(circular_number)+','+str(from_date)+','+str(to_date)+','+str(note)+','+str(min_qty)+','+str(bonus_type)+','+str(status)+',,,\n'
