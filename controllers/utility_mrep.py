...
# ==========================
    ...
    current_status_sql="select s_key, s_value, note from sm_settings where s_key='payment_mode_credit'"
    # return current_status_sql
    current_status = db.executesql(current_status_sql, as_dict=True)
    ...
    return dict(depot_transfer=depot_transfer,showLastIDType=showLastIDType,showLastID=showLastID,access_permission=access_permission, current_status=current_status)
...
def client_market_update():
    ...
    btn_client_mar_up=request.vars.btn_client_mar_up
    if btn_client_mar_up:
        # return 10
        transfer_client_id_list=request.vars.transfer_client_id_list
        # return transfer_client_id_list
        update_area_id=request.vars.update_area_id
        # return update_area_id
        transfer_password=request.vars.transfer_password
        transfer_checkbox=request.vars.transfer_checkbox
        
        if (transfer_client_id_list=='' or update_area_id==''or transfer_password=='' or transfer_checkbox!='Confirm'):
            session.flash='Required All Value accurately'
        else:
            transfer_client_id_str=''
            transfer_client_id_list = transfer_client_id_list.split('\n')
            ...
            areachk_sql="select * from sm_rep_area where cid='"+c_id+"' and area_id='"+update_area_id+"';"
            # return areachk_sql
            areachk = db.executesql(areachk_sql, as_dict=True)
            if len(areachk)>0:
                if transfer_password!='sdf23423dfdg343fdgfdz':
                    session.flash='Invalid Password'
                else:
                    areaid=str(update_area_id)
                    
                    sql="update `sm_client` set area_id='"+areaid+"' WHERE cid='"+c_id+"' and client_id in ("+transfer_client_id_str+");"
                    # return sql
                    db.executesql(sql)
                    
                    session.flash='Updated successfully in Client'
            else :
                session.flash='Invalid Area ID'
...
def doc_market_update():
    ...
    btn_doc_data=request.vars.btn_doc_data
    if btn_doc_data:
        # return 10
        doc_UP_list=request.vars.doc_UP_list
        # return doc_UP_list
        update_area_id=request.vars.update_area_id
        # return update_area_id
        transfer_password=request.vars.transfer_password
        ...
        if (doc_UP_list=='' or update_area_id==''or transfer_password=='' or transfer_checkbox!='Confirm'):
            session.flash='Required All Value accurately'
        else:
            doc_up_id_str=''
            doc_UP_list = doc_UP_list.split('\n')
            
            for i in range(len(doc_UP_list)):
                doc_id=str(doc_UP_list[i]).strip()
                if doc_id!='':
                    if doc_up_id_str=='':
                        doc_up_id_str="'"+doc_id+"'"
                    else:
                        doc_up_id_str+=",'"+doc_id+"'"
            ...
            areachk = db.executesql(areachk_sql, as_dict=True)
            if len(areachk)>0:
                for i in range(len(areachk)):
                    userRecordsStr = areachk[i]
                    area_name = str(userRecordsStr['area_name'])
                    # return area_name
                ...
                if transfer_password!='sdf23423dfdg343fdgfdz':
                    ...
                    sql="update `sm_doctor_area` set area_id='"+areaid+"',area_name='"+area_name+"' WHERE cid='"+c_id+"' and doc_id in ("+doc_up_id_str+");"
                    # return sql
                    db.executesql(sql)
                    
                    session.flash='Updated successfully in Doctor'
            else :
                session.flash='Invalid Area ID'
...
def update_cron():
    c_id=session.cid
    btn_cron=request.vars.btn_cron
    if btn_cron:
        update_sql='Update sm_depot set auto_del_cron_flag =0 where cid= "'+c_id+'" and auto_del_cron_flag =1'
        update_corn = db.executesql(update_sql)
        session.flash = 'Updated Successfully'
    redirect(URL('utility_mrep','utility_settings'))
    # return dict()


#============================= FORCE SYNCH ON FUNCTION ========================= #
def force_sync():
    c_id=session.cid
    force_sync=request.vars.btn_force_sync
    if force_sync:
        update_sql='Update sm_rep set sync_code = "" where cid= "'+c_id+'" '
        # return update_sql
        update_corn = db.executesql(update_sql)
        session.flash = 'Updated Successfully'
    redirect(URL('utility_mrep','utility_settings'))
...
def payment_mode_credit():
    
    btn_on = str(request.vars.btn_on).strip()
    session.btn_on=btn_on
    btn_off = str(request.vars.btn_off).strip()
    session.btn_off=btn_off
    active_date = str(request.vars.active_date).replace('None','')
    s_value = ''
    current_status = ''
    if btn_on =='On':
        update_sql ="UPDATE sm_settings SET s_value = 'YES' where s_key='payment_mode_credit' limit 1";
        # return update_sql
        ...
        update_sql ="UPDATE sm_settings SET s_value = 'NO', note = '"+str(active_date)+"' where s_key='payment_mode_credit' limit 1";
        # return update_sql
        update = db.executesql(update_sql)
        session.flash ="Updated Successfully"
        redirect(URL('utility_mrep', 'utility_settings'))
...
# SELECT `sm_doctor`.`id`, `sm_doctor`.`cid`, `sm_doctor`.`doc_id`, `sm_doctor`.`doc_name`, `sm_doctor`.`specialty`, `sm_doctor`.`degree`, `sm_doctor`.`password`, `sm_doctor`.`mobile`, `sm_doctor`.`sync_code`, `sm_doctor`.`des`, `sm_doctor`.`status`, `sm_doctor`.`attached_institution`, `sm_doctor`.`designation`, `sm_doctor`.`dob`, `sm_doctor`.`mar_day`, `sm_doctor`.`doctors_category`, `sm_doctor`.`service_kol_dsc`, `sm_doctor`.`service_id`, `sm_doctor`.`third_party_id`, `sm_doctor`.`pharma_route`, `sm_doctor`.`nmd_route`, `sm_doctor`.`doctors_inst`, `sm_doctor`.`chamber_1`, `sm_doctor`.`chamber_2`, `sm_doctor`.`microunion`, `sm_doctor`.`microunion_name`, `sm_doctor`.`inst_type`, `sm_doctor`.`capacity`, `sm_doctor`.`data_1`, `sm_doctor`.`data_2`, `sm_doctor`.`data_3`, `sm_doctor`.`data_4`, `sm_doctor`.`field1`, `sm_doctor`.`field2`, `sm_doctor`.`note`, `sm_doctor`.`created_on`, `sm_doctor`.`created_by`, `sm_doctor`.`updated_on`, `sm_doctor`.`updated_by`, `sm_doctor_area`.`id`, `sm_doctor_area`.`cid`, `sm_doctor_area`.`doc_id`, `sm_doctor_area`.`doc_name`, `sm_doctor_area`.`area_id`, `sm_doctor_area`.`area_name`, `sm_doctor_area`.`client_id`, `sm_doctor_area`.`client_name`, `sm_doctor_area`.`address`, `sm_doctor_area`.`district`, `sm_doctor_area`.`thana`, `sm_doctor_area`.`depot_id`, `sm_doctor_area`.`latitude`, `sm_doctor_area`.`longitude`, `sm_doctor_area`.`status`, `sm_doctor_area`.`report_status`, `sm_doctor_area`.`field1`, `sm_doctor_area`.`field2`, `sm_doctor_area`.`note`, `sm_doctor_area`.`created_on`, `sm_doctor_area`.`created_by`, `sm_doctor_area`.`updated_on`, `sm_doctor_area`.`updated_by` FROM `sm_doctor`, `sm_doctor_area` 


# SELECT `sm_doctor`.`doc_id` as doctor_id, `sm_doctor`.`doc_name` as doctor_name, `sm_doctor`.`specialty`, `sm_doctor`.`degree`, `sm_doctor`.`password`, `sm_doctor`.`mobile`, `sm_doctor`.`sync_code`, `sm_doctor`.`des`, `sm_doctor`.`status` as doc_status, `sm_doctor`.`attached_institution`, `sm_doctor`.`designation`, `sm_doctor`.`dob`, `sm_doctor`.`mar_day`, `sm_doctor`.`doctors_category`, `sm_doctor`.`service_kol_dsc`, `sm_doctor`.`service_id`, `sm_doctor`.`third_party_id`, `sm_doctor`.`pharma_route`, `sm_doctor`.`nmd_route`, `sm_doctor`.`doctors_inst`, `sm_doctor`.`chamber_1`, `sm_doctor`.`chamber_2`, `sm_doctor`.`microunion`, `sm_doctor`.`microunion_name`, `sm_doctor`.`inst_type`, `sm_doctor`.`capacity`, `sm_doctor`.`data_1`, `sm_doctor`.`data_2`, `sm_doctor`.`data_3`, `sm_doctor`.`data_4`, `sm_doctor`.`field1` as doc_field1, `sm_doctor`.`field2` as doc_field2, `sm_doctor`.`note` as doc_note, `sm_doctor_area`.`doc_id` as area_doc_id, `sm_doctor_area`.`doc_name` as area_doc_name, `sm_doctor_area`.`area_id`, `sm_doctor_area`.`area_name`, `sm_doctor_area`.`client_id`, `sm_doctor_area`.`client_name`, `sm_doctor_area`.`address`, `sm_doctor_area`.`district`, `sm_doctor_area`.`thana`, `sm_doctor_area`.`depot_id`, `sm_doctor_area`.`latitude`, `sm_doctor_area`.`longitude`, `sm_doctor_area`.`status` as doc_area_status, `sm_doctor_area`.`report_status`, `sm_doctor_area`.`field1` as area_field1, `sm_doctor_area`.`field2` as area_field1, `sm_doctor_area`.`note` as area_note FROM `sm_doctor`, `sm_doctor_area` 

def depot_pending_order_process():
    
    c_id=session.cid
    # p_depot_id=str(request.vars.pending_order_depot_id)
    
    btn_update=str(request.vars.btn_depot_process)
    # return order_sl

    if btn_update:
        # return 10
        try:
            p_depot_id=str(request.vars.pending_order_depot_id).split('|')[0]
        except:
            p_depot_id = str(request.vars.pending_order_depot_id)
            
        order_sl_list=request.vars.order_sl_list
        # doc_UP_list=request.vars.doc_UP_list
        order_sl_str=''
        order_sl_list = order_sl_list.split(',')
        # return order_sl_list
        for i in range(len(order_sl_list)):
            order_sl=str(order_sl_list[i]).strip()
            if order_sl!='':
                if order_sl_str=='':
                    order_sl_str=""+order_sl+""
                else:
                    order_sl_str+=","+order_sl+""
    
        
    
    order_update_sql= "UPDATE sm_order_head set field2 = '1' WHERE cid ='"+str(c_id)+"' AND depot_id ='"+str(p_depot_id)+"' AND sl IN ("+str(order_sl_str)+");"
    # return order_update_sql
    updat_sql_execute= db.executesql(order_update_sql)
    session.flash='successfully updated order'
    redirect(URL('utility_mrep', 'utility_settings'))
...
def depot_id_list():                                                                                                                                                                                                                                        
    ...
    depot_sql = "select depot_id,name from sm_depot group by depot_id;"
    depot_rows = db.executesql(depot_sql, as_dict=True)
    for i in range(len(depot_rows)):
        records_ov_dict=depot_rows[i]   
        depot_id=str(records_ov_dict["depot_id"])      
        depot_name=str(records_ov_dict["name"])      
        # des_degree_id=str(records_ov_dict["id"]) 
        # des=str(records_ov_dict["des"])  
        
        if reiStr == '':
            reiStr = depot_id+'|'+depot_name
        else:
            reiStr += ',' +depot_id+'|'+depot_name
