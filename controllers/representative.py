...
def rep_area_validation(form):
    ...
    # check_sql_str= "select mobile_number from base_mobile where base_id='"+str(area_id)+"';"
    # check_sql = db.executesql(check_sql_str, as_dict=True)
...
def rep_area():
    ...
    if form.accepts(request.vars,session,onvalidation=rep_area_validation):
        ...
        mob_num=''
        check_base_str=db((db.base_mobile.cid==session.cid) & (db.base_mobile.base_id==area_id) ).select(db.base_mobile.mobile_number,limitby=(0,1))          
        # return check_base_str
        if check_base_str:
            mob_num=check_base_str[0].mobile_number


        check_mobile_sql=db((db.sm_rep.cid==session.cid) & (db.sm_rep.mobile_no==mob_num) ).select(db.sm_rep.rep_id)       
        if not check_mobile_sql:   
             response.flash = 'Invalid Base ID'
        else:
...
def supervisor_level():
    ...
    if form.accepts(request.vars,session,onvalidation=validation_supervisor_level):
        mob_num=''
        check_base_str=db((db.base_mobile.cid==session.cid) & (db.base_mobile.base_id==level_id) ).select(db.base_mobile.mobile_number,limitby=(0,1))          
        # return check_base_str
        if check_base_str:
            mob_num=check_base_str[0].mobile_number
