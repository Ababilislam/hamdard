...
def home():
    ...
    # session.sch_level_search =sch_level_search
    # session.sch_zone_search =sch_zone_search
    # session.sch_area_search =sch_area_search
    # session.sch_rx_type_search =sch_rx_type_search
    # session.search_item_id_name_search =search_item_id_name_search
    ...
    # if (from_date=='None' or from_date==None):
    ...
    if (((str(session.sch_zone_search)=='') or (str(session.sch_zone_search)=='None') or (str(session.sch_zone_search)==None)) and ((str(session.sch_level_search)=='') or (str(session.sch_level_search)=='None') or (str(session.sch_level_search)==None)) and ((str(session.sch_area_search)=='') or (str(session.sch_area_search)=='None') or (str(session.sch_area_search)==None))):
        ...
        # return 't'
    # else:
    #     if str(session.sch_area_search)=='' and session.sch_area_search==None and session.sch_area_search=='None':
    #         qset = qset(db.sm_prescription_seen_details.area_id.belongs(session.level_area_id_list))
    ...
    # ===========Rx_count======
    # return session.sch_area_search
    qset_rx=db()
    ...
    # qset_show = qset_rx(db.sm_prescription_seen_details.area_id.belongs(session.level_area_id_list)).select(db.sm_prescription_seen_head.sl, groupby=db.sm_prescription_seen_head.sl)
    # qset_show =   qset_rx(db.sm_prescription_seen_details.cid == session.cid).select(db.sm_prescription_seen_head.sl, groupby=db.sm_prescription_seen_head.sl)
    

    # rx_u_count=qset_rx.count()
    # for q in qset_show:
    #     rx_u_count=rx_u_count+1
