...
def updateClientProfile():
    ...
    client_data = urllib.parse.unquote(request.vars.client_data)
...
def salsCall_order_smry():
    ...
    if (user_type=='REP'):
        ...
        for i in range(len(records_ov)): 
            ...
            qset_invoice_str="SELECT count(sl) as inv_count FROM sm_invoice_head WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced' AND area_id = '"+str(area_id)+"' AND  invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"'"
            ...
            qset_inv_amount="SELECT ((SUM((sm_invoice.price) * ( sm_invoice.quantity ) )) - (SUM((sm_invoice.price) * ( sm_invoice.return_qty ))) ) AS inv_totalprice FROM sm_invoice WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced'  AND area_id = '"+str(area_id)+"'  AND invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"'"
    ...
    if (user_type=='SUP'):
        ...
        for i in range(len(records_ov)): 
            ...
            if (level=='level0'):
                ...
                qset_invoice_str="SELECT count(sl) as inv_count FROM sm_invoice_head WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced' AND level0_id = '"+str(level_id)+"' AND  invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"'"
                # return qset_invoice_str
                reportRows_invoice_count=db.executesql(qset_invoice_str,as_dict = True)
                invoice_count=0
                for reportRows_invoice_count in reportRows_invoice_count:
                    invoice_count=reportRows_invoice_count['inv_count']
                ...
                qset_inv_amount="SELECT SUM((sm_invoice.price) * ( sm_invoice.quantity ) ) AS inv_totalprice FROM sm_invoice WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced'  AND level0_id = '"+str(level_id)+"'  AND  invoice_date >= '"+str(date_from)+"' AND  invoice_date <= '"+str(date_to)+"' " 
                reportRow_inv_amount=db.executesql(qset_inv_amount,as_dict = True)
            ...
            if (level=='level1'):
                ...
                qset_invoice_str="SELECT count(sl) as inv_count FROM sm_invoice_head WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced' AND level1_id = '"+str(level_id)+"' AND  invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"'"
                reportRows_invoice_count=db.executesql(qset_invoice_str,as_dict = True)
                ...
                qset_inv_amount="SELECT SUM((sm_invoice.price) * ( sm_invoice.quantity ) ) AS inv_totalprice FROM sm_invoice WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced'  AND level1_id = '"+str(level_id)+"'  AND  invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"' " 
                reportRow_inv_amount=db.executesql(qset_inv_amount,as_dict = True)
            ...
            if (level=='level2'):
                ...
                qset_invoice_str="SELECT count(sl) as inv_count FROM sm_invoice_head WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced' AND level2_id = '"+str(level_id)+"' AND  invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"'"
                reportRows_invoice_count=db.executesql(qset_invoice_str,as_dict = True)
                ...
                qset_inv_amount="SELECT SUM((sm_invoice.price) * ( sm_invoice.quantity ) ) AS inv_totalprice FROM sm_invoice WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced'  AND level2_id = '"+str(level_id)+"'  AND  invoice_date >='"+str(date_from)+"' AND  invoice_date <='"+str(date_to)+"' " 
                reportRow_inv_amount=db.executesql(qset_inv_amount,as_dict = True)
...
# ============================================   
            ...
            productcomRows = "SELECT item_id,name FROM sm_item WHERE cid='"+cid+"' AND status='ACTIVE' ORDER BY item_id"
            ...
            degree_row_str=db(db.doc_degree.cid == cid).select(db.doc_degree.degree_name, groupby=db.doc_degree.degree_name, orderby=db.doc_degree.degree_name)
            # return db._lastsql
            doc_degree_str=''
            for degree_row_str in degree_row_str:
                doc_degree_str=doc_degree_str+str(degree_row_str.degree_name)+','
...
#                 ----------Tourplan Market-------------------------------------------              
               ...
               #                ----------------------------Doctor list start-----------------------
                ...
                for marketRow_1 in marketRows:
                    ...
                    # doctorRowsn = "SELECT doc_id,doc_name,area_id FROM sm_doctor_area  WHERE   cid='"+cid+"'  AND area_id='"+area_id+"' AND status = 'ACTIVE' ORDER BY area_id,doc_name "
                    doctorRows = db((db.sm_doctor.cid == cid) & (db.sm_doctor.status == 'ACTIVE') & (db.sm_doctor_area.status == 'ACTIVE') & (db.sm_doctor_area.cid == cid) & (db.sm_doctor_area.doc_id == db.sm_doctor.doc_id)  & (db.sm_doctor_area.area_id == area_id) ).select(db.sm_doctor_area.doc_id, db.sm_doctor_area.doc_name, db.sm_doctor_area.area_id, orderby=db.sm_doctor_area.area_id|db.sm_doctor.doc_name)
                    # return db._lastsql
...
#                ----------------------------Doctor list start-----------------------
    ...
    #                 return len(marketList)
        ...
        #                 return db._lastsql
                        # doctorRows = db((db.sm_doctor_area.cid == cid)  & (db.sm_doctor_area.area_id==area_id)  & (db.sm_doctor_area.status=='ACTIVE')).select(db.sm_doctor_area.doc_id, db.sm_doctor_area.doc_name, db.sm_doctor_area.area_id, orderby=db.sm_doctor_area.area_id|db.sm_doctor_area.doc_name)
                        doctorRows = db((db.sm_doctor.cid == cid) & (db.sm_doctor.status == 'ACTIVE') & (db.sm_doctor_area.status == 'ACTIVE') & (db.sm_doctor_area.cid == cid) & (db.sm_doctor_area.doc_id == db.sm_doctor.doc_id)  & (db.sm_doctor_area.area_id == area_id) ).select(db.sm_doctor_area.doc_id, db.sm_doctor_area.doc_name, db.sm_doctor_area.area_id, orderby=db.sm_doctor_area.area_id|db.sm_doctor.doc_name)
                        if not doctorRows:
                            pass
        #                    retStatus = 'FAILED<SYNCDATA>Doctor not available'
...
#                 marketStrDocThisMonth=''
#                 marketStrDoc=''
                return 'SUCCESS<SYNCDATA>' + str(sync_code) + '<SYNCDATA>' + str(marketStr) + '<SYNCDATA>' + str(productStr) + '<SYNCDATA>' + str(merchandizingStr) + '<SYNCDATA>' + str(dealerStr) + '<SYNCDATA>' + str(brandStr) + '<SYNCDATA>' + str(complainTypeStr) + '<SYNCDATA>' + str(compFromStr) + '<SYNCDATA>' + str(taskTypeStr) + '<SYNCDATA>' + str(regionStr) + '<SYNCDATA>' + str(giftStr) + '<SYNCDATA>' + str(clienttCatStr) + '<SYNCDATA>' + str(clientStr) + '<SYNCDATA>' + str(menuStr)+ '<SYNCDATA>' +str(ppmStr) + '<SYNCDATA>' + str(user_type)+ '<SYNCDATA>' +str(doctorStr) + '<SYNCDATA>' +str(visit_save_limit)+'<SYNCDATA>'+str(visit_location)+'<SYNCDATA>'+str(delivery_date)+'<SYNCDATA>'+str(payment_date)+'<SYNCDATA>'+str(payment_mode)+'<SYNCDATA>'+str(collection_date)+'<SYNCDATA>'+str(promo_str)+'<SYNCDATA>'+str(client_depot)+'<SYNCDATA>'+str(cliendepot_name)+'<SYNCDATA>'+str(catStr)+'<SYNCDATA>'+str(spcStr)+'<SYNCDATA>'+str(marketStrCteam)+'<SYNCDATA>'+str(cTeam)+'<SYNCDATA>'+str(marketStrDoc)+'<SYNCDATA>'+str(marketTourStr)+'<SYNCDATA>'+str(marketStrDocThisMonth)+'<SYNCDATA>'+str(prProductStr)+'<SYNCDATA>'+str(with_whom)+'<SYNCDATA>'+str(rx_show)+'<SYNCDATA>'+str(linkStr) +'<SYNCDATA>'+str(cl_catStr) +'<SYNCDATA>'+str(cl_subcatStr) +'<SYNCDATA>'+str(repType)+'<SYNCDATA>'   +str(prSampleStr)+'<SYNCDATA>'+str(module_Str)+'<SYNCDATA>'+str(productcomStr)+'<SYNCDATA>'+str(doc_degree_str)
...
#    return client_id
    ...
    s_value = ''
    check_current_order_status_sql="SELECT s_key, s_value from sm_settings where cid = '"+cid+"' and s_key='order_status' limit 1"
    # return check_current_order_status_sql
    check_current_order_status = db.executesql(check_current_order_status_sql, as_dict=True)
    for o in range(len(check_current_order_status)):
        records_ov_dict = check_current_order_status[o]
        s_value=str(records_ov_dict['s_value'])

    if s_value == 'NO':
        return 'FAILED<SYNCDATA>Order Restricted'
    ...
    mix_flag=0
    fmcg_product_string=''
    fmcg_str = "SELECT s_value FROM sm_settings  WHERE cid='"+cid+"' AND s_key='fmcg_product_string'  LIMIT 1"
    fmcgRow=db.executesql(fmcg_str,as_dict=True)
    for j in range(len(fmcgRow)):
        fmcgRow_single=fmcgRow[j]
        fmcg_product_string = str(fmcgRow_single['s_value'])

    # return len(order_infoList)
    itemIdstr=''
    for i in range(len(order_infoList)):
        orderDataList = order_infoList[i].split('<fd>')
        if len(orderDataList) == 2:
            itemId = str(orderDataList[0])
            
            if (fmcg_product_string.find(itemId)==-1):
                general_flag=1
            if (fmcg_product_string.find(itemId)!=-1):
                fmcg_flag=1
    # return itemIdstr
    if (fmcg_flag==1 and general_flag==1):
        return 'FAILED<SYNCDATA>FMCG প্রোডাক্ট আলাদাভাবে অর্ডার করতে হবে।'
...
#                 return 'FAILED<SYNCDATA>Order Already Submitted. No need to submit again. Please check last order if needed'
    ...
    current_credit_payment_mode = ''
    activate_date = ''
    get_payment_mode_sql = "SELECT s_key, s_value, note FROM sm_settings WHERE cid = '"+cid+"' AND s_key = 'payment_mode_credit' limit 1"
    # return get_payment_mode_sql
    get_payment_mode = db.executesql(get_payment_mode_sql, as_dict = True)
    for p in range(len(get_payment_mode)):
        record = get_payment_mode[p]
        current_credit_payment_mode = str(record['s_value'])
        activate_date = str(record['note'])

    current_date =str(date_fixed).split(' ')[0]
    current_date = dt.strptime(current_date, "%Y-%m-%d")
    activate_date = dt.strptime(activate_date, "%Y-%m-%d")

    # return payment_mode
    if current_credit_payment_mode == 'NO' and payment_mode == 'CREDIT':
        if current_date >= activate_date:
            return 'FAILED<SYNCDATA> Please Select Cash Payment Mode'
    ...
    visit_date = str(current_date).split(' ')[0]
    # return visit_date
    visit_datetime = date_fixed
    firstDate = first_currentDate
            ...
            if not clientRecords:
                ...
                try:
                    ...
                    try:
                        ...
                        # depotRow[0].update_record(order_sl=ordSl)
                        # return "g"
                        test_record = cid+"','"+depot_id+"','"+depot_name+"','"+str(ordSl)+"','"+store_id+"','"+store_name+"','"+rep_id+"','"+rep_name+"','"+market_id+"','"+market_name+"','"+mobile_no+"','"+user_type+"','"+client_id+"','"+client_name+"','"+client_cat+"','"+visit_date
                        # return test_record
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
                ...
                stockBalanceRecords=db((db.sm_depot_stock_balance.cid==cid)&(db.sm_depot_stock_balance.depot_id==depot_id) & (db.sm_item.cid==cid) & (db.sm_depot_stock_balance.item_id==db.sm_item.item_id) & (db.sm_item.status=='ACTIVE')).select(db.sm_depot_stock_balance.ALL,(db.sm_depot_stock_balance.quantity-db.sm_depot_stock_balance.block_qty).sum(),db.sm_item.name,db.sm_item.unit_type,groupby=db.sm_depot_stock_balance.item_id,orderby=db.sm_item.name)
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
            ...
            # spc_row=db(db.doc_speciality.cid == cid).select(db.doc_speciality.specialty, orderby=db.doc_speciality.specialty    )
            spc_row=db(db.doc_speciality.cid == cid).select(db.doc_speciality.specialty, groupby=db.doc_speciality.specialty, orderby=db.doc_speciality.specialty)
            spcStr=''
            for spc_row in spc_row:
                spcStr=spcStr+str(spc_row.specialty)+','
#             cat_row=db(db.doc_catagory.cid == cid).select(db.doc_catagory.category, orderby=db.doc_catagory.category)
...
def doc_info_submit():
    ...
    dMicroUnion=str(request.vars.dcMicrounion).strip()
...
def chemist_info_submit():
    ...
    Address_Line_check=str(request.vars.Address_Line_1).strip() 
    Address_Line_1=check_special_char_address(Address_Line_check)
    ...
    SubCategory='Not Applicable|0'#str(request.vars.SubCategory).strip()
    ...
    cl_pocketMarket_edt=str(request.vars.cl_pocketMarket_edt).strip()
    # return SubCategory
...
#     thana=urllib.unquote(thana.decode('utf8'))
    ...
    subcatName=''
    cl_pocketMarket_id=''
    cl_pocketMarket_name=''

    if Category!='' and Category!=None and Category!='None':
        catID=Category.split('|')[1]
        catName=Category.split('|')[0]

    if SubCategory!='' and SubCategory!=None and SubCategory!='None':
        subcatID=SubCategory.split('|')[1]
        subcatName=SubCategory.split('|')[0]


    if cl_pocketMarket_edt!='' and cl_pocketMarket_edt!=None and cl_pocketMarket_edt!='None':
        cl_pocketMarket_id=cl_pocketMarket_edt.split('|')[1]
        cl_pocketMarket_name=cl_pocketMarket_edt.split('|')[0]
    ...
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
            ...
            if chemistRow:  
                 db((db.sm_client_temp.cid == cid) & (db.sm_client_temp.client_id == client_id) ).update( address=Address_Line_1,area_id =route,market_id =route,vat_registration_no=RegistrationNo ,district=district,thana=thana,p_market_id=cl_pocketMarket_id,p_market_name=cl_pocketMarket_name,nid=int(NID) , owner_name=Contact_Name ,contact_no1=Contact_phone ,category_id=catID , category_name=catName,sub_category_id=subcatID ,sub_category_name=subcatName, dob=DOB ,field1=Cash_Credit ,credit_limit=str(Credit_Limit), status = Status, newChemist=0)
                ...
                db.sm_client_temp.insert(cid =cid,client_id=client_id ,name=ChemistName ,address=Address_Line_1,area_id =route,market_id =route,vat_registration_no=RegistrationNo ,district=district,thana=thana,nid=int(NID) ,p_market_id=cl_pocketMarket_id,p_market_name=cl_pocketMarket_name, owner_name=Contact_Name ,contact_no1=Contact_phone ,category_id=catID , category_name=catName,sub_category_id=subcatID ,sub_category_name=subcatName, field1=Cash_Credit ,credit_limit=str(Credit_Limit), status = Status, newChemist=0)
                        ...
                        if new_doc==1 and pharma_route_id!='':
                            ...
                            if not phrmaChkRow:
                                ...
                                if docRowCheckP:
                                    ...
                                    docarea_insert=db.sm_doctor_area.insert(cid=cid,doc_id=doc_id,doc_name=doc_name,area_id=pharma_route, status='ACTIVE')    
                                ...
                                if docRowCheckS:
                                    ...
                                    docarea_insert=db.sm_doctor_area.insert(cid=cid,doc_id=doc_id,doc_name=doc_name,area_id=sin_route_id,status='ACTIVE')    
...
#                                 delete_temp=db((db.sm_doctor_temp.cid == cid) & (db.sm_doctor_temp.doc_id == docID)).delete()
                            ...
                            doc_update=db((db.sm_doctor.cid == cid) & (db.sm_doctor.doc_id == doc_id) ).update(doc_name=doc_name, specialty=specialty,degree=degree,mobile=mobile,des=des,status='ACTIVE',attached_institution=attached_institution,designation=designation,dob=dob,mar_day=mar_day,doctors_category=doctors_category,service_kol_dsc=service_kol_dsc,service_id=service_id,third_party_id=third_party_id,field1=microunion)#,pharma_route=pharma_route_id,nmd_route=nmd_route_id)
                            docRowCheck=db((db.sm_doctor_area.cid == cid) & (db.sm_doctor_area.doc_id == doc_id) & (db.sm_doctor_area.area_id == pharma_route_id) ).select(db.sm_doctor_area.area_id, limitby=(0,1))
                            if docRowCheck:
                                pass
                            else:
...
#     client_id = str(request.vars.client_id).strip()
    ...
    cl_pocketMarket=str(request.vars.cl_pocketMarket).strip()
...
#     cl_pocketMarket=urllib.unquote(cl_pocketMarket.decode('utf8'))
    # return cl_pocketMarket
    catID=''
    catName=''
    subcatID=''
    subcatName=''
    ...
    if cl_pocketMarket!='' and cl_pocketMarket!=None and cl_pocketMarket!='None':
        p_market_id=cl_pocketMarket.split('|')[1]
        p_market_name=cl_pocketMarket.split('|')[0]
...
#                 pass
            # return 'sdf'
            db.sm_client_temp.insert(cid =cid,client_id=maxClient_id ,address=Address_Line_1,area_id =route,p_market_id=p_market_id,p_market_name=p_market_name,name=ChemistName ,vat_registration_no=RegistrationNo ,district=district,thana=thana,nid=int(NID) , owner_name=Contact_Name ,contact_no1=Contact_phone ,category_id=catID , category_name=catName,sub_category_id=subcatID ,sub_category_name=subcatName, field1=Cash_Credit ,credit_limit=int(Credit_Limit) ,status=Status,photo=imageName, field2=NumberofDoc,note=AvgPatientPerDay,newChemist=1)
            # return db._lastsql
...
#                     DOB             =chemistRows[0].dob
                    ...
                    p_market_id     =chemistRows[0].p_market_id
                    p_market_name   =chemistRows[0].p_market_name
...
#                     return depoRow
                    ...
                    if repRow:
                        ...
                        depot_belt_name=depoRow[0].depot_name
...
#                             db((db.sm_client.cid == cid) & (db.sm_client.client_id == client_id) ).update( area_id =route,name=ChemistName,address=address ,vat_registration_no=RegistrationNo ,district=district,thana=thana,nid=int(NID) , owner_name=Contact_Name ,contact_no1=Contact_phone ,category_id=catID , category_name=catName,sub_category_id=subcatID ,sub_category_name=subcatName, dob=DOB ,field1=Cash_Credit ,credit_limit=int(Credit_Limit) ,status='ACTIVE',photo=imageName,field2=NumberofDoc,note=AvgPatientPerDay,depot_id=depot_id,depot_name=depot_name,store_id=store_id,store_name=store_name)
                            db((db.sm_client.cid == cid) & (db.sm_client.client_id == client_id) ).update( area_id =route,name=ChemistName,address=address ,vat_registration_no=RegistrationNo ,district=district,thana=thana,nid=int(NID) , owner_name=Contact_Name ,contact_no1=Contact_phone ,category_id=catID , category_name=catName,sub_category_id=subcatID ,sub_category_name=subcatName ,field1=Cash_Credit ,credit_limit=int(Credit_Limit) ,status='ACTIVE',photo=imageName,field2=NumberofDoc,note=AvgPatientPerDay,depot_id=depot_id,depot_name=depot_name, depot_belt_name = depot_belt_name, store_id=store_id,store_name=store_name,p_market_id =p_market_id,p_market_name=p_market_name)
...
def addDodNmd():  
    ...
    add_data=urllib.parse.unquote(add_data)
    separator='<url>'    
...
#============================= Test dynamic path
def dmpath():
    # return '<start>http://127.0.0.1:8000/navana/syncmobile_pocketmarket/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://127.0.0.1:8000/navana/syncmobile_new_design_raw_sql/<fd>http://127.0.0.1:8000/navana/syncmobile_new_design_raw_sql/<fd>http://127.0.0.1:8000/navana/tour_web/<fd>http://127.0.0.1:8000/navana/tour_web_members/<end>'
    
   # ====apu
    return '<start>http://127.0.0.1:8000/hamdard/syncmobile_pocketmarket/<fd>http://i001.yeapps.com/image_hub/static/mundi_image/<fd>http://i001.yeapps.com/image_hub/mundi_image/<fd>http://127.0.0.1:8000/hamdard/syncmobile_pocketmarket/<fd>http://127.0.0.1:8000/hamdard/syncmobile_pocketmarket/<fd>http://127.0.0.1:8000/hamdard/tour_web/<fd>http://127.0.0.1:8000/hamdard/tour_web_members/<end>'
    # =====apu end
