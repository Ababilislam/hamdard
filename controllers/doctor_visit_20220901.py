...
def download_doctor_visit():
    ...
    if (session.btn_filter_docvisit):
        ...
        # return   session.from_dt  
        if (session.from_dt!='' and session.from_dt!=None and session.to_date!='' and session.to_date!=None):
            qset = qset(db.sm_doctor_visit.visit_date >= session.from_dt)
            qset = qset(db.sm_doctor_visit.visit_date <= session.to_date)
