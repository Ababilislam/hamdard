...
def depot_market_add():
    ...
    if btn_filter_depot_market:
        session.btn_filter_depot_market=request.vars.btn_filter_depot_market
        session.depot_market_branch_id_search=request.vars.depot_market_branch_id_search
        session.depot_market_id_search=request.vars.depot_market_id_search
        reqPage=0

    elif btn_all:
        session.btn_filter_depot_market=None
        session.depot_market_branch_id_search=None
        session.depot_market_id_search=None
        reqPage=0
    ...
    if session.user_type=='Depot':
        ...
        if (session.btn_filter_depot_market!=None and session.depot_market_branch_id_search!=None and session.depot_market_branch_id_search!=''):
            depot_market_branch_id_search=str(session.depot_market_branch_id_search).split('|')[0]          
            qset=qset(db.sm_depot_market.depot_id==depot_market_branch_id_search)    

        if (session.btn_filter_depot_market!=None and session.depot_market_id_search!=None and session.depot_market_id_search!=''):
            depot_market_id_search=str(session.depot_market_id_search).split('|')[0]          
            qset=qset(db.sm_depot_market.market_id==depot_market_id_search)    
...
def depot_market_download():
    ...
    records=qset.select(db.sm_depot_market.ALL,db.sm_depot.name,db.sm_depot.short_name,orderby=db.sm_depot_market.depot_id|db.sm_depot_market.market_id)
...
def get_depot_branch_name_id():

    depotBranchh = ''
    cid = session.cid        
    rows = db((db.sm_depot.cid == cid)).select(db.sm_depot.depot_id, db.sm_depot.name, orderby=db.sm_depot.name, groupby=db.sm_depot.name)

    for row in rows:
        depot_id = str(row.depot_id).replace('|', ' ').replace(',', ' ')        
        name = str(row.name).replace('|', ' ').replace(',', ' ')
        if depotBranchh == '':
            depotBranchh = depot_id + '|' + name            
        else:
            depotBranchh += ',' + depot_id + '|' + name
    return depotBranchh

def get_depot_market_name_id():

    depotMarket = ''
    cid = session.cid        
    rows = db((db.sm_depot_market.cid == cid)).select(db.sm_depot_market.market_id, db.sm_depot_market.market_name, orderby=db.sm_depot_market.market_name, groupby=db.sm_depot_market.market_name)

    for row in rows:
        market_id = str(row.market_id).replace('|', ' ').replace(',', ' ')        
        market_name = str(row.market_name).replace('|', ' ').replace(',', ' ')
        if depotMarket == '':
            depotMarket = market_id + '|' + market_name            
        else:
            depotMarket += ',' + market_id + '|' + market_name
    return depotMarket
