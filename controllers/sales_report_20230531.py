...
# http://127.0.0.1:8000/hamdard/sales_report/salesDcr_report_detail?cid=HAMDARD&rep_id=6478&password=1234&synccode=4109
...
def salesDcr_report_rsm_url():
    ...
    session.from_dt = from_dt
    session.to_date = to_date
    # return session.from_dt,session.to_date
...
def salesDcr_report_rsm():
    ...
    # return to_date
    # return route_id,from_date,to_date
    date_from=current_date
    now = datetime.datetime.strptime(current_date, "%Y-%m-%d")
    date_ton=now - datetime.timedelta(days = 1)
    date_to=str(date_ton).split(' ')[0]
    ...
    if (user_type=='SUP'):
        ...
        if (from_dt and to_date)!='' and (from_dt and to_date)!=None and (from_dt and to_date)!='None':                    
            ...
            to_date_1=now + datetime.timedelta(days = 1)
            to_date=str(to_date_1).split(' ')[0]
            qset=qset((db.sm_doctor_visit.visit_date >= from_date) & (db.sm_doctor_visit.visit_date < to_date))
            # return from_date
        else:
            qset = qset((db.sm_doctor_visit.visit_date <= from_date) and (db.sm_doctor_visit.visit_date > to_date))
            # session.from_dt=date_to
            # session.to_date=date_from
        # return qset
        ...
        qsetCount = qsetCount((db.sm_doctor_visit.visit_date >= from_date) and (db.sm_doctor_visit.visit_date < to_date))
        # return qsetCount
        if (from_dt and to_date)!='' and (from_dt and to_date)!=None:                    
            qsetCount=qsetCount((db.sm_doctor_visit.visit_date >= from_date) & (db.sm_doctor_visit.visit_date < to_date))
...
def salesDcr_report_fm():
    ...
    # return db._lastsql
    # return records
    return dict(doc_id_list=doc_id_list,records=records,to_date=to_date,cid=cid,rep_id=rep_id,password=password,synccode=synccode,vChecklist=vChecklist,vCountList=vCountList,search_form=search_form)
...
def slsRptDcr_area_wise_url():
    ...
    session.route_id = route_id 
    session.level2_id = level2_id 
    session.level2_name = level2_name 
    session.from_dt = from_dt
    session.to_date = to_date
...
def salesDcr_report_detail():
    ...
    if (user_type=='REP'):
        ...
        records = qset.select(db.sm_doctor_visit.doc_id.count(),db.sm_doctor_visit.doc_id,db.sm_doctor_visit.rep_id,db.sm_doctor_visit.level3_id,db.sm_doctor_visit.level3_name,db.sm_doctor_visit.visit_date,db.sm_doctor_visit.level2_id,db.sm_doctor_visit.level2_name, orderby=db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.route_id)
        # return records
        doc_id_list=[]
        visit_date_list=[]
        if records:
    ...
    if (user_type=='SUP'):
        ...
        if (from_dt and to_date)!='' and (from_dt and to_date)!=None:   
            # return to_date
            qset=qset((db.sm_doctor_visit.visit_date >= from_dt) & (db.sm_doctor_visit.visit_date <= date_to))
        else:
            qset = qset((db.sm_doctor_visit.visit_date <= date_from) and (db.sm_doctor_visit.visit_date > to_date))

        records = qset.select(db.sm_doctor_visit.doc_id.count(),db.sm_doctor_visit.rep_id,db.sm_doctor_visit.doc_id,db.sm_doctor_visit.level3_id,db.sm_doctor_visit.level3_name,db.sm_doctor_visit.visit_date,db.sm_doctor_visit.level2_id,db.sm_doctor_visit.level2_name, orderby=~db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.route_id)
        # records = qset.select(db.sm_doctor_visit.doc_id,db.sm_doctor_visit.rep_id,db.sm_doctor_visit.doc_id,db.sm_doctor_visit.level3_id,db.sm_doctor_visit.level3_name,db.sm_doctor_visit.visit_date,db.sm_doctor_visit.level2_id,db.sm_doctor_visit.level2_name, orderby=~db.sm_doctor_visit.visit_date, groupby=db.sm_doctor_visit.route_id)
        # return date_to
        # return db._lastsql
        doc_id_list=[]
        visit_date_list=[]
...
def slsRptDcr_area_wise():
    ...
    # return route_id
    level2_id =session.level2_id
    level2_name =session.level2_name
    from_dt=session.from_dt
    to_date=session.to_date
    # return from_dt
    ...
    if (user_type=='REP'):
        ...
        condition=condition+" AND s1.visit_date >= '"+ str(from_dt) +"' AND s1.visit_date <= '"+ str(to_date) +"' "    
        ...
        sql_str="SELECT  COUNT(s1.doc_id)as doc_count,s1.level3_id as level3_id,level3_name,s1.doc_id as doc_id, s1.doc_name as doc_name,s1.id as id,s1.visit_dtime as visit_dtime,s1.giftnsample as giftnsample  FROM sm_doctor_visit as s1  WHERE s1.cid = '"+ str(cid) +"' AND s1.rep_id = '"+ str(rep_id) +"' "+ condition + " GROUP BY s1.route_id,s1.id;"
        # return sql_str
        records_ov=db.executesql(sql_str,as_dict = True)
    ...
    if (user_type=='SUP'):
        ...
        records_ov=[]
        if int(depth)==0:
            sql_str="SELECT  COUNT(dv.doc_id)as doc_count,dv.doc_id as doc_id, dv.doc_name as doc_name,dv.id as id,dv.visit_dtime as visit_dtime,dv.giftnsample as giftnsample,dv.level3_id as level3_id,level3_name FROM sm_doctor_visit as dv  WHERE dv.cid = '"+ str(cid) +"'  AND dv.visit_date >= '"+ str(from_dt) +"' AND dv.visit_date <= '"+ str(to_date) +"'  AND dv.level2_id = '"+ str(route_id) +"' GROUP BY dv.id"
            # return sql_str
        elif int(depth)==1:
            sql_str="SELECT  COUNT(dv.doc_id)as doc_count,dv.doc_id as doc_id, dv.doc_name as doc_name,dv.id as id,dv.visit_dtime as visit_dtime,dv.giftnsample as giftnsample,dv.level3_id as level3_id,level3_name FROM sm_doctor_visit as dv  WHERE dv.cid = '"+ str(cid) +"'  AND dv.visit_date >= '"+ str(from_dt) +"' AND dv.visit_date <= '"+ str(to_date) +"'  AND dv.level2_id = '"+ str(route_id) +"' GROUP BY dv.id"
        elif int(depth)==2:
    ...
    return dict(date_to=date_to,level2_id=level2_id,level2_name=level2_name,cid=cid,rep_id=rep_id,password=password,synccode=synccode,records_ov=records_ov,route_id=route_id)#,visit_date=visit_date)
