...
def doctor_visit_add():
    ...
    from_date_v = str(request.vars.from_dt).strip().upper()    
    to_date_v= str(request.vars.to_dt).strip().upper()
    ...
    if btn_filter_docvisit:
        ...
        dateFlag=True
        try:
            fromDate=datetime.datetime.strptime(str(from_date_v),'%Y-%m-%d').strftime('%Y-%m-%d')
            session.from_date_v=fromDate
        except:
            session.from_date_v=''
            
        try:
            toDate=datetime.datetime.strptime(str(to_date_v),'%Y-%m-%d').strftime('%Y-%m-%d')
            session.to_date_v=toDate
        except:
            session.to_date_v=''
        reqPage=0
                
    elif btn_all:
        session.from_date_v=None
        session.to_date_v=None
    ...
    # # return session.btn_filter_docvisit
    if (session.btn_filter_docvisit):
                
        if (session.from_date_v!='' and session.from_date_v!=None and session.to_date_v!='' and session.to_date_v!=None):
            qset = qset(db.sm_doctor_visit.visit_date >= session.from_date_v)
            qset = qset(db.sm_doctor_visit.visit_date <= session.to_date_v)
        ...
        # return qset   
        elif (session.searchType_docvisit == 'AreaID'):
            searchValue=str(session.searchValue_docvisit).split('|')[1]
            qset = qset(db.sm_doctor_visit.level2_name == searchValue)

        elif (session.searchType_docvisit == 'ZoneID'):
            searchValue=str(session.searchValue_docvisit).split('|')[1]
            # return searchValue
            qset = qset(db.sm_doctor_visit.level1_name == searchValue)

        elif (session.searchType_docvisit == 'DivisionID'):
            searchValue=str(session.searchValue_docvisit).split('|')[1]
            qset = qset(db.sm_doctor_visit.level0_name == searchValue)
        ...
        elif (session.searchType_docvisit == 'Gift'):
            qset = qset(db.sm_doctor_visit.giftnsample == session.searchValue_docvisit)

        elif (session.searchType_docvisit == 'PPM'):
            qset = qset(db.sm_doctor_visit.giftnsample == session.searchValue_docvisit)

        elif (session.searchType_docvisit == 'Sample'):
            qset = qset(db.sm_doctor_visit.giftnsample == session.searchValue_docvisit)
    ...
    if (session.from_date_v=='' or session.from_date_v==None):        
        search_form.vars.from_dt=''
    else:
        search_form.vars.from_dt=session.from_date_v
        
    if (session.to_date_v=='' or session.to_date_v==None):        
        search_form.vars.to_dt=''
    else:
        search_form.vars.to_dt=session.to_date_v
    
    if search_form.accepts(request.vars,session):
        pass

    return dict(search_form=search_form,form=form, records=records,totalCount=totalCount, page=page, items_per_page=items_per_page, access_permission=access_permission)
...
def download_doctor_visit():
    ...
    records = qset.select(db.sm_doctor_visit.ALL, orderby= ~db.sm_doctor_visit.visit_dtime)
    # return records
    totalCount=qset.count()

    #---------
    myString='Doctor Visit Plan \n'
    myString+='Count: '+str(totalCount)+' \n\n'
    myString+='Visit date,Rep ID,Rep Name,Doctor ID,Doctor Name,Market ID,Market Name,Depot,Feedback,Status\n'
    for rec in records:
        ...
        feedback=str(rec.feedback)
        ho_status=str(rec.ho_status)
         
        myString+=visit_date +','+rep_id+','+rep_name+','+doc_id+','+doc_name+','+route_id+','+route_name+','+depot_id+','+feedback+','+ho_status+'\n'
...
def get_gift_list():
    regStr = ''
    cid = session.cid
    rows = db(db.sm_doctor_gift.cid == cid).select(db.sm_doctor_gift.gift_id, db.sm_doctor_gift.gift_name, orderby=db.sm_doctor_gift.gift_name)
    for row in rows:
        gift_id = str(row.gift_id)
        gift_name = str(row.gift_name).replace('|', ' ').replace(',', ' ')

        if regStr == '':
            regStr = gift_id + '|' + gift_name
        else:
            regStr += ',' + gift_id + '|' + gift_name
...
def get_ppm_list():
    repStr = ''
    cid = session.cid
    rows = db(db.sm_doctor_ppm.cid == cid).select(db.sm_doctor_ppm.gift_id, db.sm_doctor_ppm.gift_name, orderby=db.sm_doctor_ppm.gift_name)
    for row in rows:
        gift_id = str(row.gift_id)
        gift_name = str(row.gift_name).replace('|', ' ').replace(',', ' ')

        if repStr == '':
            repStr = gift_id + '|' + gift_name
        else:
            repStr += ',' + gift_id + '|' + gift_name
...
def get_smp_list():
    resmStr = ''
    cid = session.cid
    rows = db(db.sm_doctor_sample.cid == cid).select(db.sm_doctor_sample.item_id, db.sm_doctor_sample.name, orderby=db.sm_doctor_sample.name)
    for row in rows:
        gift_id = str(row.item_id)
        gift_name = str(row.name).replace('|', ' ').replace(',', ' ')

        if resmStr == '':
            resmStr = gift_id + '|' + gift_name
        else:
            resmStr += ',' + gift_id + '|' + gift_name
