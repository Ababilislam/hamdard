...
def inv_preview_dist():
    ...
    if distRow:
        ...
        qset = qset(db.sm_invoice_head.invoice_date >= startDt)
        ...
        # -------------------------
    return dict(data_List=data_List, dist_id=dist_id, dist_name=dist_name,startDt=startDt,endDt=endDt )
...
def return_note_preview_dist():
    ...
    if distRow:
        ...
        depot_id_list = []
        if depot_id == '':
            distRow1 = db((db.sm_depot_distributor.cid == c_id) & (db.sm_depot_distributor.dist_id == dist_id)).select(db.sm_depot_distributor.depot_id, db.sm_depot_distributor.dist_name,db.sm_depot_distributor.dist_disc_percent)
            # return distRow1
            # return db._lastsql
