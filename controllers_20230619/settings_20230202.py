...
def base_mobile2():  
    ...
    if submit:
        ...
        # return userid
        if base_id!='' and mobile_number!='':
            # return userid
            check_userid_sql = "select base_id from base_mobile where base_id='"+str(base_id)+"';"
    ...
    if session.btn_filter:
        base_id=search_value.split('|')[0].upper()
        session.base_id=base_id
        userRows_sql = "select base_id, mobile_number from base_mobile where base_id='"+str(base_id)+"' order by base_id;"
    else:
