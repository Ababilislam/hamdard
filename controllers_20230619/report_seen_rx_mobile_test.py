...
def home():
    ...
    if session.sch_rx_type!='' and session.sch_rx_type!=None:
        # return '5'
        # qset = qset(%str(db.sm_prescription_seen_head.rx_type==session.sch_rx_type))
    ...
    if session.sch_rx_type!='' and session.sch_rx_type!=None:
        qsetH = qsetH(db.sm_prescription_seen_head.rx_type==session.sch_rx_type)

    recordsH=qsetH.count()
    
    return dict(zone_name=zone_name,reg_name=reg_name,tl_name=tl_name,records=records,recordsH=recordsH,level_area_list=level_area_list,seen_rx_type_list=seen_rx_type_list)
