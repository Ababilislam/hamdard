...
def invoice_head_d():
   ...
   # store_id,store_name,order_sl,
    ...
    myString += 'Depot Id,Depot Name,SL,Order DateTime, Delivery Date,Client Id, Client Name,Rep Id, Rep Name,Area,Status,Shipment No,Actual Total TP,Total Amount,Discount,Collection Amount,Return TP,Return Discount,Invoice Date,Dist Discount\n'
    ...
    for i in range(len(records)):
        ...
        myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_datetime) + ',' + str(
            delivery_date) + ',' + str(client_id) + ',' + str(client_name) + ',' + str(rep_id) + ',' + str(
            rep_name) + ',' + str(level3_id)+ ',' + str(status)+ ',' + str(shipment_no)+ ',' + str(actual_total_tp)+ ',' + str(total_amount)+ ',' + str(discount)+ ',' + str(collection_amount)+ ',' + str(return_tp) + ',' + str(return_discount)+ ',' + str(invoice_date)+ ',' + str(dist_discount) + '\n'
...
def invoice_detail_d():
   ...
   # store_id,store_name,order_sl,
    records = "SELECT depot_id,depot_name,sl,order_datetime,delivery_date,client_id,client_name,rep_id,rep_name,level3_id,invoice_date,item_id,item_name,category_id,actual_tp,quantity,bonus_qty,price,return_qty,status,dist_discount_item FROM sm_invoice where cid = '" + c_id + "' AND order_datetime >= '" + str(from_dt)+ "' AND order_datetime <= '" + str(to_dt)+ "';"
    # return records
    records = db.executesql(records, as_dict=True)        
    ...
    myString += 'Depot Id,Depot Name,SL,Order DateTime, Delivery Date,Client Id, Client Name,Rep Id, Rep Name,Area,Invoice Date,Item ID, Item Name,Category,Actual TP,Quantity,Bonus Quantity, Price, Return Quantity,Status,Dist Discount Item\n'
    ...
    for i in range(len(records)):
        ...
        myString += str(depot_id) + ',' + str(depot_name) + ',' + str(sl) + ',' + str(order_datetime) + ',' + str(
            delivery_date) + ',' + str(client_id) + ',' + str(client_name) + ',' + str(rep_id) + ',' + str(
            rep_name) + ',' + str(level3_id)+ ',' + str(invoice_date)+ ',' + str(item_id)+ ',' + str(item_name)+ ',' + str(category_id)+ ',' + str(actual_tp)+ ',' + str(quantity)+ ',' + str(bonus_qty) + ',' + str(price)+ ',' + str(return_qty)+ ',' + str(status) + ',' + str(dist_discount_item) +'\n'
