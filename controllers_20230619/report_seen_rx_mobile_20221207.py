...
def index():
    ...
    if not userRecords:
        ...
        if user_type=='sup':
            ...
            if not zLevelRows:
                ...
                if level_depth_no==0: 
                    # return '5'                   
                    level3_Rows = db((db.sm_level.cid == c_id) & (db.sm_level.level0.belongs(z_level_list)) & (db.sm_level.is_leaf == '1') ).select(db.sm_level.level1,db.sm_level.level1_name, orderby=db.sm_level.level1,groupby=db.sm_level.level1)
                    # return level3_Rows
 ...
 # =========RX Type=============
    ...
    # search_item_id_name = []        
    #     itemRows=db((db.sm_prescription_seen_details.cid==c_id) & (db.sm_prescription_seen_details.medicine_name==search_item_id_name)).select(db.sm_prescription_seen_details.medicine_name,db.sm_prescription_seen_details.medicine_id)
    #     # return rx_typeRows
    #     if not itemRows:
    #         response.flash = 'Rx Type Not Available'
    #     else:
    #         for itemRow in itemRows:
    #             itemRow_name=itemRow.name                
    #             dictData = {'name': itemRow_name}
    #             search_item_id_name.append(dictData)

    #     session.search_item_id_name=search_item_id_name        
       
    #     search_item_id_name = []
    #     for i in range(len(search_item_id_name)):
    #         search_item_id_name = search_item_id_name[i]
    #         search_item_id_name.append(search_item_id_name['area_id'])
...
def home():
    ...
    # rec_item = db(db.sm_item.cid == session.cid).select(db.sm_item.item_id,db.sm_item.name, orderby=db.sm_item.item_id,groupby=db.sm_item.item_id)
    rec_item = db(db.sm_prescription_seen_details.cid == session.cid).select(db.sm_prescription_seen_details.medicine_name, orderby=db.sm_prescription_seen_details.medicine_name,groupby=db.sm_prescription_seen_details.medicine_name)
    ...
    if btn_report:
        ...
        session.sch_level = sch_level
        session.sch_zone = sch_zone
        session.sch_area = sch_area
        session.sch_rx_type = sch_rx_type
        session.search_item_id_name = search_item_id_name
    ...
    # level_row=  db((db.sm_prescription_seen_head.cid == session.cid) and (db.sm_prescription_seen_head.area_id == session.sch_area)).select(db.sm_prescription_seen_head.zone_name,db.sm_prescription_seen_head.reg_name,db.sm_prescription_seen_head.tl_name, limitby=(0,1))
    # # return level_row
    # if level_row:
    #     zone_name=level_row[0].zone_name
    #     reg_name=level_row[0].reg_name
    #     tl_name=level_row[0].tl_name  
    ...
    if session.sch_zone!='' and session.sch_zone!=None:
        qset = qset(db.sm_prescription_seen_head.reg_id==session.sch_zone)

    if session.sch_level!='' and session.sch_level!=None:
        qset = qset(db.sm_prescription_seen_head.tl_id==session.sch_level)


    if session.sch_area!='' and session.sch_area!=None:
        qset = qset(db.sm_prescription_seen_details.area_id==session.sch_area)
    if session.sch_rx_type!='' and session.sch_rx_type!=None:
        qset = qset(db.sm_prescription_seen_head.rx_type.like('%'+session.sch_rx_type+'%') )
    #====
    # return session.search_item_id_name
    if session.search_item_id_name!='' and session.search_item_id_name!=None:
        qset = qset(db.sm_prescription_seen_details.medicine_name==search_item_id_name)
    ...
    if session.sch_zone!='' and session.sch_zone!=None:
        qsetH = qsetH(db.sm_prescription_seen_head.reg_id==session.sch_zone)

    if session.sch_level!='' and session.sch_level!=None:
        qsetH = qsetH(db.sm_prescription_seen_head.tl_id==session.sch_level)


    if session.sch_area!='' and session.sch_area!=None:
        qsetH = qsetH(db.sm_prescription_seen_details.area_id==session.sch_area)
    if session.sch_rx_type!='' and session.sch_rx_type!=None:
        qsetH = qsetH(db.sm_prescription_seen_head.rx_type.like('%'+session.sch_rx_type+'%') )
    #====
    # return session.search_item_id_name
    if session.search_item_id_name!='' and session.search_item_id_name!=None:
        qsetH = qsetH(db.sm_prescription_seen_details.medicine_name==search_item_id_name)
    ...
    return dict(rec_item=rec_item,search_item_id_name=search_item_id_name,user_type=user_type,records=records,recordsH=recordsH,zlevel_list=zlevel_list,level_list=level_list,level_area_list=level_area_list,seen_rx_type_list=seen_rx_type_list)

# def get_item_id_name_list():
#     restStr = ''
#     cid = session.cid
        
#     rows = db(db.sm_item.cid == cid).select(db.sm_item.item_id,db.sm_item.name, orderby=db.sm_item.item_id,groupby=db.sm_item.item_id)
#     # return db._lastsql
