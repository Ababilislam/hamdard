...
def download_sample():
    ...
    records = qset.select(db.sm_doctor_sample.ALL, orderby=db.sm_doctor_sample.name)

    #---------
    myString = 'Sample List\n\n'
    myString += 'Sample ID,Name,Description,Status\n'
    for rec in records:
        itemId = str(rec.item_id)
        Name = str(rec.name).replace(',', ' ')
        des = str(rec.des).replace(',', ' ')
        status = str(rec.status)

        myString += itemId + ',' + Name + ',' + des + ',' + status + '\n'

    #-----------
    import gluon.contenttype
    response.headers['Content-Type'] = gluon.contenttype.contenttype('.csv')
    response.headers['Content-disposition'] = 'attachment; filename=download_sample.csv'
    return str(myString)
