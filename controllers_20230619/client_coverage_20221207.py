...
def client_coverage():    
    ...
    if not repRow:
       ...
       return retStatus
        ...
        if session.btn_filter:
            ...
            client_RecordsStr = "SELECT count(distinct(client_id)) as unique_client_cover  FROM sm_order_head WHERE cid = '" + str(cid) + "' AND rep_id = '" + str(rep_id) + "' AND order_date >= '" + str(date_from) + "' AND order_date <= '" + str(date_to) + "' " + condition + " GROUP BY client_id;"
            # return client_RecordsStr 
            client_Records = db.executesql(client_RecordsStr, as_dict=True)

            unique_client_cover = 0
            for i in range(len(client_Records)):
                client_RecordsStr = client_Records[i]
                unique_client_cover = client_RecordsStr['unique_client_cover']
                unique_client_cover+=int(unique_client_cover) 


            #=============Client Cover Percentage=============== 
            
             
            unique_client_cover= (int(total_client) * int(unique_client_cover))
            unique_client_cover_per=int(unique_client_cover/100)
            ...
            inv_count = 0   
            invRecords_str = "SELECT count(distinct(client_id)) as inv_count FROM sm_invoice_head WHERE cid = '" + str(cid) + "' AND rep_id = '" + str(rep_id) + "' AND invoice_date >= '" + str(date_from) + "' AND invoice_date <= '" + str(date_to) + "' " + condition + "  AND return_count!='1' GROUP BY client_id;"
            # return invRecords_str
            invRecords = db.executesql(invRecords_str, as_dict=True)
            
            for i in range(len(invRecords)):
                invRecords_str = invRecords[i]
                inv_count = invRecords_str['inv_count']
    ...
    return dict(cid=cid,rep_id=rep_id,rep_pass=rep_pass,records_ov=records_ov,total_client=total_client,unique_client_cover=unique_client_cover,unique_client_cover_per=unique_client_cover_per,inv_count=inv_count)
