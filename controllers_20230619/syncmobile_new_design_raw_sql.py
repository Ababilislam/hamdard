...
#     AvgPatientPerDay=urllib.unquote(AvgPatientPerDay.decode('utf8'))
    ...
    if SubCategory!='' or Category!=None or Category!='None':
        subcatID=SubCategory.split('|')[1]
        subcatName=SubCategory.split('|')[0]
    if DOB=='' or DOB==None or DOB=='None':
        DOB='000-00-00'
    ...
    if cl_pocketMarket!='' or cl_pocketMarket!=None or cl_pocketMarket!='None':
        pockter_market_ID=cl_pocketMarket.split('|')[0]
        pockter_market_Name=cl_pocketMarket.split('|')[1]
    # return pockter_market_ID, pockter_market_Name
#    return  subcatID
#     mName=dDist.split('|')[0]
#     mID=dDist.split('|')[1]
...
#     Status=urllib.unquote(Status.decode('utf8'))
#     thana=urllib.unquote(thana.decode('utf8'))
    # return catName, catID, subcatName, subcatID
    if (DOB==''): 
        DOB='1900-01-01'
    compRow = db((db.sm_company_settings.cid == cid) & (db.sm_company_settings.status == 'ACTIVE')).select(db.sm_company_settings.cid, limitby=(0, 1))
    if not compRow:
        ...
        if not repRow:
           return 'FAILED<SYNCDATA>Invalid Authorization'
            ...
            if userType=='rep':
                ...
                chemistRows = db((db.sm_client_temp.cid == cid) & (db.sm_client_temp.status != 'REJECTED') & (db.sm_client_temp.status != 'INACTIVE') & (db.sm_client_temp.newChemist == 0) & (db.sm_client_temp.name != 'NULL') & (db.sm_client_temp.name != 'None') & (db.sm_client_temp.name != None)).select(db.sm_client_temp.id,db.sm_client_temp.client_id,db.sm_client_temp.name, db.sm_client_temp.address, db.sm_client_temp.contact_no1,db.sm_client_temp.trade_license_no,db.sm_client_temp.vat_registration_no,db.sm_client_temp.photo,db.sm_client_temp.note,db.sm_client_temp.status, orderby=db.sm_client_temp.client_id|~db.sm_client_temp.client_id, groupby=db.sm_client_temp.client_id)
                # return doctorRows
                if not chemistRows:
                    pass
...
#                     DOB             =chemistRows[0].dob
                    ...
                    # return p_market_id, p_market_name
