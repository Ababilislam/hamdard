...
def prescription_list():
    ...
    date_from = str(request.vars.date_from).strip().replace('None','')
    date_to = str(request.vars.date_to).strip().replace('None','')
    ...
    if btn_filter_prescription:
        ...
        # return session.date_from,date_to
...
#        qset=qset(db.sm_prescription_seen_head.depot_id==session.depot_id)
    ...
    if (session.btn_filter_prescription):
        if session.date_from!='' and session.date_to != '':
            qset = qset(db.sm_prescription_seen_head.submit_date >= session.date_from)
...
#     ============Download
    if btn_download:
        ...
        search_year = str(request.vars.search_year).strip()

        if ((search_month=='') and (search_year=='')):
            response.flash='Required Year Month'
        else:
            # from_date=first_currentDate
            current_year=str(search_year).split('-')[0]
    #         return current_year
            if search_month=='JAN':
            ...
            if ((search_month!='') and (search_year!='')):
                ...
                myString+='Date  ,SL,  Region ID,  Region Name,Zone ID,  Zone Name, Area ID, Area Name ,Teritory ID, Teritory Name ,  SubmitBy ID, Submit By Name, Rx Type, Brand Name ,Doctor ID, Doctor Name,\n' #medicineID, medicineName, medType
                ...
                for i in range(len(recordList)):
                    ...
                    try:
                        doctor_name=str(doctor_name).replace(',',' ')
                    except:
                        doctor_name=doctor_id


                                
                    myString+=str(submit_date)+','+str(sl)+','+str(level0)+','+str(level0_name)+','+str(level1)+','+str(level1_name)+','+str(level2)+','+str(level2_name)+','+str(level3)+','+str(level3_name)+','+str(submit_by_id)+','+str(submit_by_name)+','+str(rx_type)+','+str(medicine_name)+','+str(doctor_id)+','+str(doctor_name)+'\n'
                    #-----------
