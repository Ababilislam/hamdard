...
#                         
                        ...
                        if session.user_type =='Admin':
                            menuDivStr += """<div style=" background-color:#F2F2F2; border-radius:0px; padding:0px 0px 0px 0px; margin:0px; font-size:16px; height:25px; vertical-align:middle; background-image: linear-gradient( #F5F5F5 , #CACACA 50px);"><div id="tools">&nbsp;TempData</div></div>"""
                            menuDivStr += """<div style="margin:0px; padding:0px; border:0px;">"""
                            menuDivStr += """<ul>"""
                            
                            if check_role('rm_device_manage') == True or check_role('rm_device_view') == True:
                                 menuDivStr += """<li ><a href="/""" + appName + """/temp_data/client"><div>Client</div></a></li>"""
                                 
                            if check_role('rm_user_log_view') == True:
                                 menuDivStr += """<li ><a href="/""" + appName + """/temp_data/doctor"><div>Doctor</div></a></li>"""
...
def get_id_list():
    ...
    id_list_sql = "select rep_id,name from sm_rep where user_type = 'rep'  group by rep_id order by rep_id;"
    # return id_list_sql
...
def get_status_list():
    ...
    return ret_status_Str
                 ...
                 ######### CLIENT LIST AUTO COMPLETE USING AJAX ###########

# def get_client_id_list():
#     ret_client_Str = ''
#     get_client_sql = "SELECT client_id, name FROM sm_client WHERE cid = 'HAMDARD' AND status = 'ACTIVE' group by client_id "
#     # return get_client_sql
#     client_list = db.executesql(get_client_sql, as_dict=True)
#     for i in range(len(client_list)):
#         records_ov=client_list[i]   
#         client_id=str(records_ov["client_id"])      
#         name=str(records_ov["name"])      
#         if ret_client_Str == '':
#             ret_client_Str = client_id+'|'+name
#         else:
#             ret_client_Str += ',' +client_id+'|'+name
    
#     return ret_client_Str
...
def get_client_id_list():
    ...
    for record in client_list:
        ...
        name = record["name"]
        if name is not None:
            name = name.encode("utf-8")  # Encode the name using UTF-8
            name = name.decode("utf-8")  # Decode the name back to Unicode
        else:
            name = ""  # Set an empty string if name is None
        if ret_client_Str == '':
            ret_client_Str = client_id + '|' + name
        else:
            ret_client_Str += ',' + client_id + '|' + name
