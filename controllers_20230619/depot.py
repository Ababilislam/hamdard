...
#========================= Date filter start ============
    ...
    if (session.btn_filter_issue):
        ...
        elif (session.search_type_issue=='PROCESS_STATUS'):
            qset=qset(db.sm_issue_head.issue_process_status==session.search_value_issue)
...
def validation_damage(form):
    ...
    # depot_id=str(form.vars.depot_id)
    # depot_name=str(form.vars.depot_id)
    
    depot_idName=form.vars.depot_id 

    depot_ID=str(depot_idName).split('|')[0]
    depot_Name=str(depot_idName).split('|')[1]

    form.vars.depot_id=depot_ID
    form.vars.depot_Name=depot_Name
        # return depot_ID,depot_Name

    store_idName=form.vars.store_id     
    store_ID=str(store_idName).split('|')[0]
    store_Name=str(store_idName).split('|')[1]    
    ...
    if store_ID=='':
        ...
        # if not(store_id_to=='' or store_id_to=='|'):
...
#   --------------------- 
    ...
    if form.accepts(request.vars,session,onvalidation=validation_damage):
        ...
        depot_idNAME=request.vars.depot_id
        
        depot_id=str(depot_idNAME).split('|')[0]
        depot_name=str(depot_idNAME).split('|')[1]

        store_idNAME=request.vars.store_id
...
def item_list():                                                                                                                                                                                                                                        
    ...
    itemRows_sql = "select item_id,name from sm_item where status='active';"
    # return itemRows_sql
