...
def collection_list():
    ...
    if (session.user_type!='Depot'):
        ...
        if (session.btn_filter_payment):
            ...
            #------------
            if not (session.mso_id=='' or session.mso_id==None):
                mso_id=str(session.mso_id).split('|')[0]
                #qset=qset(db.sm_invoice_head.area_id==territory_id)
                condStr+=" and rep_id='"+str(mso_id)+"'"
...
def download_payment_list():
    ...
    # =============AUTO COMPLETE USING AJAX for MSO/SUP===================#

def mso_list():                                                                                                                                                                                                                                        
    retStr = ''

    msoRows_sql = "select rep_id,name from sm_rep where status='active';"
    # return msoRows_sql
    msoRows = db.executesql(msoRows_sql, as_dict=True)
    for i in range(len(msoRows)):
        records_ov_dict=msoRows[i]   
