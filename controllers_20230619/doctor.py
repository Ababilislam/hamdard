...
def download_doctor():
    ...
    for rec in records:
        ...
        docName = rec.sm_doctor.doc_name.replace(',', ' ').replace('\n', ' ')
        specialty = rec.sm_doctor.specialty.replace(',', '; ').replace('\n', ' ')
        degree = rec.sm_doctor.degree.replace(',', '; ').replace('\n', ' ')
        chamberAddress = rec.sm_doctor.des.replace(',', '; ').replace('\n', ' ')
        attachInstitute = rec.sm_doctor.attached_institution.replace(',', '; ').replace('\n', ' ')
        designation = rec.sm_doctor.designation.replace(',', '; ').replace('\n', ' ')
        dob = rec.sm_doctor.dob          
        mobile = rec.sm_doctor.mobile
        doctorsCategory = rec.sm_doctor.doctors_category.replace(',', '; ').replace('\n', ' ')
        status = rec.sm_doctor.status.replace(',', ' ').replace('\n', ' ')
        updated_by = rec.sm_doctor.updated_by
        area_id = rec.sm_doctor_area.area_id.replace(',', ' ').replace('\n', ' ')
        area_name = rec.sm_doctor_area.area_name.replace(',', ' ').replace('\n', ' ')
        if dob == 'None':
            dob = ''
        # if int(mobile) == 0:
        #     mobile = ''
