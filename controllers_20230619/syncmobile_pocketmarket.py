...
def salsCall_order_smry():
    ...
    if (user_type=='REP'):
        ...
        for i in range(len(records_ov)): 
            ...
            qset_inv_amount="SELECT ((SUM((sm_invoice.price) * ( sm_invoice.quantity ) )) - (SUM((sm_invoice.price) * ( sm_invoice.return_qty ))) ) AS inv_totalprice FROM sm_invoice WHERE cid =  '"+ str(cid) +"' AND status = 'Invoiced'  AND area_id = '"+str(area_id)+"'  AND order_datetime>='"+str(date_from)+"' AND  order_datetime<='"+str(date_to)+"'"
