        ...
        if dateFlag==False:
            ...
            if dateDiff>90:
                ...
                if dateFlag==False:
                    ...
                    dateDiff=(to_dt-from_dt2).days
                    # return dateDiff
                    if dateDiff>15:                        
                        response.flash="Maximum 15 days allowed between Date Range"
                    else:                        
                        if btn_invoice_head_d:
                            redirect (URL('invoice_head_d',vars=dict(fromDate=from_dt,toDate=to_dt))) 

                        if btn_invoice_detail_d:
                            redirect (URL('invoice_detail_d',vars=dict(fromDate=from_dt,toDate=to_dt))) 
...
def invoice_detail_d():
   ...
   # store_id,store_name,order_sl,
    records = "SELECT depot_id,depot_name,sl,order_datetime,delivery_date,client_id,client_name,rep_id,rep_name,level3_id,invoice_date,item_id,item_name,category_id,actual_tp,quantity,bonus_qty,price,return_qty,status,dist_discount_item FROM sm_invoice where cid = '" + c_id + "' AND order_datetime >= '" + str(from_dt)+ "' AND order_datetime <= '" + str(to_dt)+ "' order by order_datetime;"
    # return records
    records = db.executesql(records, as_dict=True)        
