       ...
       return retStatus
        ...
        if ((session.btn_filter) and (session.date_from!='') and (session.date_to!='')):
            ...
            if levelIdstr != '':
                ...
                client_RecordsStr = "SELECT count(distinct(client_id)) as unique_client_cover  FROM sm_invoice_head WHERE cid = '" + str(cid) + "'   AND invoice_date >= '" + str(date_from) + "' AND invoice_date <= '" + str(date_to) + "' " + condition1 + " AND return_count!='1' GROUP BY client_id,invoice_date;" 
                ...
                client_RecordsStr = "SELECT count(distinct(client_id)) as unique_client_cover  FROM sm_invoice_head WHERE cid = '" + str(cid) + "'  AND invoice_date >= '" + str(date_from) + "' AND invoice_date <= '" + str(date_to) + "' " + condition + "  AND return_count!='1' GROUP BY client_id,invoice_date;"                
                # return client_RecordsStr
                client_Records = db.executesql(client_RecordsStr, as_dict=True)
        ...
        if not ((session.btn_filter) and (levelIdstr=='')):    
            ...
            sql_str="SELECT client_id,name FROM sm_client WHERE cid = '"+ str(cid) +"' AND area_id = '" + str(levelIdstr) + "'  GROUP BY client_id order by name asc;"        
            
            records_ov=db.executesql(sql_str,as_dict = True)    

            inv_count = 0   
            invRecords_str = "SELECT count(client_id) as total_inv_count,level3_id,client_id,invoice_date FROM sm_invoice_head WHERE cid = '" + str(cid) + "'  AND invoice_date >= '" + str(date_from) + "' AND invoice_date <= '" + str(date_to) + "' AND area_id = '" + str(levelIdstr) + "'  AND return_count!='1' GROUP BY client_id;"
